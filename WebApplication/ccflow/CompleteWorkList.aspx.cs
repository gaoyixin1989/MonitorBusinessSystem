...
public partial class ccflow_CompleteWorkList : PageBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(opt))
        {
            switch (opt)
            {
                case "getCompleteWorkList":
                    Response.Write(getCompleteWorkList());
                    break;
                default: break;
            }
    ...
    protected string getCompleteWorkList()
    {
        ...
        {
            queryParams.Add(new string[3] { "FlowEnderRDT", Request["endRDTStart"], "ge" });
        }
        if (!string.IsNullOrEmpty(Request["endRDTEnd"]))
        {
            queryParams.Add(new string[3] { "FlowEnderRDT", Request["endRDTEnd"], "le" });
        }
        ...
        string info = CCFlowFacade.GetCompleateWork(user.USER_NAME, queryParams.ToArray(), page, pageSize);
