...
public partial class ccflow_RuningWorkList : PageBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(opt))
        {
            switch (opt)
            {
                case "getRuningWorkList":
                    Response.Write(getRuningWorkList());
                    break;
                default: break;
            }
    ...
    protected string getRuningWorkList()
    {
        ...
        string info = CCFlowFacade.GetRunning(user.USER_NAME, queryParams.ToArray(), page, pageSize);
