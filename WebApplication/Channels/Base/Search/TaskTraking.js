...
function showTaskInfo() {
    ...
    $.ajax({
        type: "POST",
        async: false,
        url: "TaskTraking.aspx?type=getFlowInfo&business_id=" + selectedTaskItem.ID,
        contentType: "application/json; charset=utf-8",
        success: function (data) {
            if (data != null) {
                winWidth = data;
    ...
    var strUrl = ("TaskFlowImg.aspx?task_id=" + selectedTaskItem.ID);
    $(document).ready(function () { $.ligerDialog.open({ title: '任务追踪', url: strUrl, width: winWidth, height: 600, modal: false }); });
}
