...
var strMyPage = "SummarySheet_Lab.aspx";
...
function showSubTaskGrid(strTaskId) {
    ...
    function upLoadSummaryTable() {
        ...
        $.ligerDialog.open({ title: '汇总表上传', width: 500, height: 270,
            ...
            }], url: '../../../OA/ATT/AttFileUpload.aspx?filetype=ResultSummaryTable&id=' + subTaskGrid.getSelectedRow().ID
        });
    }
}
...
function excelExportForTask_Summary() {
    ...
    detailExcelWin = $.ligerDialog.open({ url: "../SearchFunction/print_Summary.aspx?isLocal=0&task_id=" + selectedTaskItem.ID });
}
...
function excelExportForSubTask_Summary() {
    ...
    detailExcelWin = $.ligerDialog.open({ url: "../SearchFunction/print_Summary.aspx?isLocal=0&task_id=" + selectedTaskItem.ID + "&subtask_id=" + selectedsubTaskItem.ID });
}
...
function showSheetTypeDialog() {
    ...
    $.ajax({
        ...
        type: "POST",
        url: "SummarySheet_Lab.aspx?action=GetSheetType&subTaskID=" + selectedsubTaskItem.ID,
    ...
    function excelExportFor_SubTask_Summary() {
        ...
        if (strSheetType != "") {
            ...
            detailExcelWin = $.ligerDialog.open({ isHidden: false, url: "../SearchFunction/print_Summary.aspx?isLocal=0&task_id=" + selectedTaskItem.ID + "&subtask_id=" + selectedsubTaskItem.ID + "&sheetType=" + strSheetType });
        }
    }
}