...
$(document).ready(function () {
    ...
    firstManager = $("#firstgrid").ligerGrid({
        ...
        onCheckRow: function (checked, rowdata, rowindex) {
            for (var rowid in this.records)
                this.unselect(rowid);
            this.select(rowindex);
            selectedConItem = firstManager.getSelectedRow();
            //点击的时候加载点位数据
            secondManager.set('url', "TotalSearch.aspx?action=GetTaskInfo&contract_id=" + rowdata.ID);
        },
    ...
    menu1 = $.ligerMenu({ width: 120, items:
            ...
            { id: 'task', text: '任务追踪', click: clickForTask, icon: 'role' },
             { id: 'report', text: '报告', click: clickForTask, icon: 'pager' }
            ]
    });
    //监测任务grid
    secondManager = $("#secondgrid").ligerGrid({
        ...
        onCheckRow: function (checked, rowdata, rowindex) {
            for (var rowid in this.records)
                this.unselect(rowid);
            this.select(rowindex);
            selectedTaskItem = secondManager.getSelectedRow();
        },
        onBeforeCheckAllRow: function (checked, grid, element) { return false; }
    });
    $(".l-grid-hd-cell-btn-checkbox").css("display", "none"); //隱藏checkAll
...
function itemclick_OfToolbar_UnderItem(item) {
    switch (item.id) {
        case 'srh':
            ...
            break;
        case 'srhAll':
            firstManager.set('url', "TotalSearch.aspx?Action=GetContractInfo");
            break;
        case 'contract':
            showContractInfo();
            break;
...
function clickForTask(item) {
    switch (item.id) {
        case 'srhAll':
            secondManager.set('url', "TotalSearch.aspx?action=GetTaskInfo");
            break;
        case 'sample':
            showSampleInfo();
            break;
...
function showDetailSrh() {
    if (detailWinSrh) {
        ...
        mainform.ligerForm({
            ...
            fields: [
                      ...
                      { display: "委托书编号", name: "srhContractCode", newline: false, type: "text" },
    ...
    function search() {
        ...
        firstManager.set('url', "TotalSearch.aspx?Action=GetContractInfo&SrhContractType=" + SrhContractType + "&SrhContractCode=" + SrhContractCode + "&SrhProjectName=" + SrhProjectName + "&SrhYear=" + SrhYear);
    }
}
...
function showContractInfo() {
    ...
    var selectedConItem = firstManager.getSelectedRow();
    if (!selectedConItem) {
...
////////////////报告生成
function showReport() {
    var selectedTaskItem = secondManager.getSelectedRow();
    if (!selectedTaskItem) {
        $.ligerDialog.warn('请先选择一条数据！');
        return;
    }
