...
$(document).ready(function () {
    ...
    $("#searchForm").ligerForm({
        ...
        fields: [
                      ...
                      { display: "仪器名称", name: "ddlNAME", newline: false, type: "text" }
                    ]
    });
    ...
    objGrid = $("#grid").ligerGrid({
        ...
        columns: [
                ...
                { display: '样品名称', name: 'SAMPLE_NAME', minWidth: 100 },
                { display: '监测项目', name: 'ITEM_NAME', minWidth: 120 },
                { display: '分析负责人', name: 'REAL_NAME', minWidth: 120 },
                { display: '仪器名称', name: 'NAME', minWidth: 200 },
                { display: '仪器开始使用时间', name: 'APPARTUS_START_TIME', minWidth: 140 },
                { display: '仪器结束使用时间', name: 'APPARTUS_END_TIME', minWidth: 140 },
                { display: '最近检定/校准时间', name: 'BEGIN_TIME', minWidth: 140 },
                { display: '到期检定/校准时间', name: 'END_TIME', minWidth: 140 },
                { display: '是否超期完成', name: 'Is_OverTime', minWidth: 100 }
                ],
        toolbar: { items: [
...
function GetSearchData() {
    ...
    var Name = encodeURI($("#ddlNAME").val());
    if (StartTime != "") {
        if (EndTime == "") {
    ...
    objGrid.set('url', url + "?action=GetData&StartTime=" + StartTime + "&EndTime=" + EndTime + "&HEAD_USERID=" + HEAD_USERID + "&TICKET_NUM=" + TICKET_NUM + "&OverTime=" + OverTime + "&NAME=" + Name); //客户端分页
}
//查询后，清除条件数据
