...
public partial class Channels_OA_EmployeTrain_EmployeTrain_LeaderAduit : PageBaseForWF, IWFStepRules
{
    ...
    void IWFStepRules.CreatAndRegisterBusinessData()
    {
        ...
        if (!String.IsNullOrEmpty(task_id))
        {
            ...
            if (dt.Rows.Count > 0)
            {
                objItems.APP_ID = dt.Rows[0]["ID"].ToString();
            }
            objItems.APP_INFO = this.hidLeaderOption.Value.ToString();
            objItems.APP_DATE = this.hidDate.Value.ToString();
