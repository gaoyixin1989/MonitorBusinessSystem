...
public partial class Channels_OA_EmployeTrain_ExamployeTrain_Aduit : PageBaseForWF, IWFStepRules
{
    ...
    void IWFStepRules.CreatAndRegisterBusinessData()
    {
        ...
        if (!String.IsNullOrEmpty(task_id) && String.IsNullOrEmpty(strBtnType))
        {
            ...
            objItems.APP_FLOW = "归档";
            objItems.APP_INFO = this.hidLeaderOption.Value;
            objItems.APP_DATE = DateTime.Now.ToString();
            objItems.APP_ID = LogInfo.UserInfo.ID;
            if (new TOaTrainPlanLogic().Edit(objItems))
            {
                wfControl.SaveInstStepServiceData("年度员工培训ID", "task_id", task_id, "2");
            }
            else
            {
        ...
        else
        {
            if (!String.IsNullOrEmpty(task_id) && strBtnType == "back")
            {
                ...
                if (new TOaTrainPlanLogic().Edit(objItems))
                {
                    wfControl.SaveInstStepServiceData("年度员工培训ID(退回)", "task_id", task_id, "B");
                }
                else
                {
                    LigerDialogAlert("数据发送失败！", "error");
