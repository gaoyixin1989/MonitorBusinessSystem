...
public partial class Channels_OA_Part_PartPlan_DeptAduit : PageBaseForWF, IWFStepRules
{
    private string task_id = "", strTaskProjectName = "", strBtnType = "";
    ...
    void IWFStepRules.CreatAndRegisterBusinessData()
    {
        ...
        if (!String.IsNullOrEmpty(task_id) && String.IsNullOrEmpty(strBtnType))
        {
            ...
            if (!String.IsNullOrEmpty(task_id) && strBtnType == "back")
            {
                ...
                if (new TOaPartBuyRequstLogic().Edit(objItems))
                {
                    wfControl.SaveInstStepServiceData("采购计划ID(退回)", "task_id", task_id, "B");
                }
                else
                {
                    LigerDialogAlert("数据发送失败！", "error");
