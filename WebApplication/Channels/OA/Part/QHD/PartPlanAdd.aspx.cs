...
public partial class Channels_OA_Part_QHD_PartPlanAdd : PageBaseForWF, IWFStepRules
{
    private string task_id = "",  strTaskProjectName = "" ,type = "";
    HttpContext context;
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!String.IsNullOrEmpty(task_id))
        {
            ...
            //注册名称
            wfControl.ServiceName = strTaskProjectName+"采购计划单";
        }

        if (!string.IsNullOrEmpty(Request.QueryString["keshi"]))
        {
            //判断哪个科室
            this.keshi.Value = Request.QueryString["keshi"].ToString();
        }
    }
    /// <summary>
    /// 获取页面隐藏域参数
