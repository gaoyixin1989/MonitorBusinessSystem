...
public partial class Channels_OA_PersonnelFile_Administration_ExamInfor_Aduit : PageBaseForWF, IWFStepRules
{
    ...
    void IWFStepRules.CreatAndRegisterBusinessData()
    {
        ...
        if (!String.IsNullOrEmpty(task_id) && String.IsNullOrEmpty(strBtnType))
        {
            ...
            objItems.DEPT_APP = this.hidDeptOption.Value.ToString();
            if (new TOaExamineInfoLogic().Edit(objItems))
            {
                wfControl.SaveInstStepServiceData("事业单位人员人事考核ID(单位审核)", "task_id", task_id, "2");
            }
            else
            {
        ...
        {
            if (!String.IsNullOrEmpty(task_id) && !String.IsNullOrEmpty(strBtnType) && strBtnType == "back")
            {
                TOaExamineInfoVo objItems = new TOaExamineInfoVo();
                objItems.ID = task_id;
                objItems.EXAMINE_STATUS = "-1";
                if (new TOaExamineInfoLogic().Edit(objItems))
                {
                    wfControl.SaveInstStepServiceData("行政事业人员考核ID(退回)", "task_id", task_id, "B");
                }
                else
                {
                    LigerDialogAlert("数据发送失败！", "error");
