...
public partial class Channels_OA_PersonnelFile_Administration_ExamInfor_Examine : PageBaseForWF, IWFStepRules
{
    ...
    void IWFStepRules.CreatAndRegisterBusinessData()
    {
        ...
        if (!String.IsNullOrEmpty(task_id))
        {
            ...
            objItems.ID = task_id;
            objItems.LEADER_APP_DATE = DateTime.Now.ToString();
            objItems.LEADER_APP = this.hidOption.Value.ToString();
            if (new TOaExamineInfoLogic().Edit(objItems))
            {
                wfControl.SaveInstStepServiceData("事业单位人员人事考核ID(考核单位审核)", "task_id", task_id, "3");
            }
            else
            {
