...
$(document).ready(function () {
    ...
    if (isAdd == true & cacheSaveFrim == false) {
        ...
        $("#Company_NameFrim").autocomplete("../../../Mis/Contract/MethodHander.ashx?action=GetCompanyInfo",
    {
        max: 12,     // 列表里的条目数 
    ...
    $("#btn_Copy").bind("click", function () {
        if (strCompanyName == "") {
            ...
            var strCompanyNameNew = $("#Company_Name").val();
            var strContactNameNew = $("#Contacts_Name").val();
            var strTelPhoneNew = $("#Tel_Phone").val();
            var strAddressNew = $("#Address").val();
            var Area_NameNew = $("#AREA_OP").val();
            $("#Company_NameFrim").val(strCompanyNameNew);
            $("#Contacts_NameFrim").val(strContactNameNew);
            $("#Tel_PhoneFrim").val(strTelPhoneNew);
            $("#AddressFrim").val(strAddressNew);
            //            $("#Industry_NameFrim").ligerGetComboBoxManager().setValue(strIndustryId);
            $("#Area_NameFrim").ligerGetComboBoxManager().setValue(Area_NameNew);
        }
        else {
...
function InsertCompanyFrimValue() {
    $.ajax({
        ...
        async: false, //设置是否为异步加载,此处必须
        type: "POST",
        url: encodeURI("../../../Mis/Contract/MethodHander.ashx?action=InsertCompany&strCompanyName=" + strCompanyNameFrim + "&strIndustryId=" + strIndustryIdFrim + "&strAreaId=" + strAreaIdFrim + "&strContactName=" + strContactNameFrim + "&strTelPhone=" + strTelPhoneFrim + "&strAddress=" + strAddressFrim + ""),
...
//        $("#Industry_NameFrim").ligerGetComboBoxManager().setValue(strIndustryIdFrim);
    ...
    function checkIsExistFrimValue() {
        $.ajax({
            ...
            async: false, //设置是否为异步加载,此处必须
            type: "POST",
            url: encodeURI("../../../Mis/Contract/MethodHander.ashx?action=checkCompany&strCompanyName=" + strCompanyNameFrim + ""),
            contentType: "application/text; charset=utf-8",
            dataType: "text",
            success: function (data) {
                if (data != "") {
