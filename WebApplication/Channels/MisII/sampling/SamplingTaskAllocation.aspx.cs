...
public partial class Channels_MisII_sampling_SamplingTaskAllocation : PageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["type"] == "check")
            {
                ...
                if (string.IsNullOrEmpty(strSendInfo))
                {
                    Response.Write("false主采人未设置完整");
    ...
    private string GetSendInfoOld(string strWorkID)
    {
    ...
    private string getThreeGridInfo(string strTwoGridId)
    {
        ...
        {
            if (dtSample.Rows[i]["REMARK5"].ToString().Length > 0)
                dtSample.Rows[i]["REMARK5"] = new TSysUserLogic().Details(dtSample.Rows[i]["REMARK5"].ToString()).REAL_NAME;
            else
                dtSample.Rows[i]["REMARK5"] = "请选择";

            if (dtSample.Rows[i]["REMARK4"] != null && dtSample.Rows[i]["REMARK4"].ToString().Length > 0)
            {
                var nameList = new List<string>();

                var idList = dtSample.Rows[i]["REMARK4"].ToString().Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                ...
                dtSample.Rows[i]["REMARK4"] = string.Join(",", nameList);
