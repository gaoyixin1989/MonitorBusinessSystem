...
$(document).ready(function () {
    ...
    //隐藏页面的发送人模块 并将发送人 默认为项目负责人 验收类别定制功能
    var acceptanceCode = ""; // 默认的验收委托类型
    $.ajax({
        type: "POST",
        async: false,
    ...
    if (acceptanceCode == $("#CONTRACT_TYPE").val()) {
        $("#cphData_wfControl_dvToOpUsers").hide();
        $.ajax({
            type: "POST",
            async: false,
            ...
            success: function (data) {
                if (data != null) {
                    var objSelector = $.parseJSON(data.d)[0];
                    $("#cphData_PROJECT_ID").val(objSelector.ID);
                }
            }
        });
    }
    //环境质量特殊功能 到该流程则直接跳过报告流程 完成任务
    $.ajax({
        type: "POST",
        async: false,
        url: url + "/GetValidateEnvMonitor",
        data: "{'strValue':'" + $("#cphData_TASK_ID").val() + "'}",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        success: function (data) {
            if (data != null && data.d == "true") {//是环境质量类别
                //$("#cphData_wfControl_dvToOpUsers").hide();
                $("#cphData_wfControl_dvToOpUsers").empty();
                $("#cphData_wfControl_btnSend").val("完成");
            }
        }
    });
    //////////////////////////////////////////////////////////////////////监测结果汇总表 动态生成//////////////////////////////////////////////////////
    ...
    secondManager = $("#secondgrid").ligerGrid({
        ...
        toolbar: { items: [
                ...
                { line: true },
                { id: 'excel', text: '导出', click: excelExport, icon: 'excel' }
                ]
        },
...
//汇总表导出
function excelExport(item) {
    //构建Json数据 回传
    $.ligerDialog.open({ url: "../print.aspx?task_id=" + $("#cphData_TASK_ID").val() });
}