...
/// 创建人：邵世卓
/// </summary>
public partial class Channels_Mis_Report_ReportZZ_ReportAccept : PageBaseForWF, IWFStepRules
{
    ...
    bool IWFStepRules.BuildAndValidateBusinessData(out string strMsg)
    {
        ...
        objTask.ID = this.TASK_ID.Value;
        objTask.COMFIRM_STATUS = "3";
        if (new TMisMonitorTaskLogic().Edit(objTask))
        {
            WriteLog("监测数据汇总表发送成功", "", LogInfo.UserInfo.ID + "监测数据汇总表" + objTask.ID + "发送成功");
            return true;
        }
        else
        {
