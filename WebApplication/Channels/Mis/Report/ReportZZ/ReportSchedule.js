...
//    }
        ...
        sampleManager = $("#sampleDiv").ligerGrid({
            ...
            {display: '样品号', name: 'SAMPLE_CODE', width: 60, align: 'left', isSort: false },
            { display: '样品名称', name: 'SAMPLE_NAME', width: 260, align: 'left', isSort: false },
            { display: '样品类型', name: 'SAMPLE_TYPE', width: 50, align: 'left', isSort: false }
...
function ReportClick() {
    ...
    if (reportId != null && reportId != "") {
        ...
        if (temId != null && temId != "")
            url = "?File_Type=.doc&Protect=0&EDIT_TYPE=1&ReportWf=ReportSchedule&Template_Id=" + temId + "&Task_Id=" + taskId + "&ITEM_TYPE=" + rptItemtypeID;
        else {
...
//    $.post("ReportSchedule.aspx?type=saveinfo&task_id=" + $("#ID").val() + "&report_code=" + $("#REPORT_CODE").val() + "&conclusion=" + encodeURIComponent($("#PROJECT_CONCLUSION").val()), function (data) {
    //    });
    $.post("ReportSchedule.aspx?type=saveinfo&task_id=" + $("#ID").val() + "&report_code=" + $("#REPORT_CODE").val() + "&conclusion=", function (data) {
    });
}
//发送保存
function SendSave() {
