...
namespace n3
{
    ...
    /// 创建人：邵世卓
    /// </summary>
    public partial class Channels_Mis_Report_ReportAccept : PageBase
    {
        ...
        {
            string strReportId = Request.QueryString["report_id"];//报告ID
            string strRptAskDate = Request.QueryString["rptaskdate"]; //要求完成时间
            TMisMonitorReportVo objReport = new TMisMonitorReportVo();
            objReport.ID = strReportId;
            objReport.RPT_ASK_DATE = strRptAskDate;
            objReport.IF_SEND = "1";
            if (new TMisMonitorReportLogic().Edit(objReport))
            {
