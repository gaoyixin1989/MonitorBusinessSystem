...
$(document).ready(function () {
    ...
    firstManager = $("#firstgrid").ligerGrid({
        ...
        url: 'ReportList_bak.aspx?action=getReportInfo',
...
function itemclick_OfToolbar_UnderItem(item) {
    switch (item.id) {
        case 'srh':
            ...
            break;
        case 'srhAll':
            firstManager.set('url', "ReportList_bak.aspx?action=getReportInfo");
            break;
        case 'Do':
            DoReport();
            break;
...
function showDetailSrh() {
    ...
    function search() {
        ...
        firstManager.set('url', "ReportList_bak.aspx?action=getReportInfo&srhContractType=" + SRH_CONTRACT_TYPE + "&srhContractCode=" + SRH_CONTRACT_CODE + "&srhProjectName=" + encodeURI(SRH_PROJECT_NAME));
    }
...
function GetContractType(code, type) {
    ...
    $.ajax({
        ...
        type: "POST",
        url: "ReportList_bak.aspx/GetDataDictName",
...
function GetClientName(id) {
    ...
    $.ajax({
        ...
        async: false, //设置是否为异步加载,此处必须
        type: "POST",
        url: "ReportList_bak.aspx/GetClientName",
        data: "{'strValue':'" + id + "'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
...
function DoReport() {
    ...
    var surl = '../Sys/WF/WFStartPage.aspx?action=|id=' + rowSelected.ID + '&WF_ID=RPT_QHD';
