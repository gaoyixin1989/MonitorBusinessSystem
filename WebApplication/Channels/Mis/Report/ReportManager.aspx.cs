...
/// 创建人：邵世卓
/// </summary>
public partial class Channels_Mis_Report_ReportManager : PageBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        //输入结果
        string strReturn = "";
        if (!string.IsNullOrEmpty(Request.QueryString["action"]) && Request.QueryString["action"] == "getNotReportInfo")
        {
            strReturn = getReportInfo("0");
            Response.Write(strReturn);
            Response.End();
        }
        if (!string.IsNullOrEmpty(Request.QueryString["action"]) && Request.QueryString["action"] == "getHasReportInfo")
        {
            strReturn = getReportInfo("1");
            Response.Write(strReturn);
            Response.End();
        }
    ...
    /// </summary>
    /// <returns></returns>
    protected string getReportInfo(string strIF_GET)
    {
        string result = "";
        int intTotalCount = 0;
        //页数
    ...
    protected void EditReportStatus()
    {
        ...
        objReport.REMARK1 = DateTime.Now.ToString();//何海亮修改，领取时间
