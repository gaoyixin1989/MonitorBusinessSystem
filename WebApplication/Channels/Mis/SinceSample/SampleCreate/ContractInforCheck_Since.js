...
$(document).ready(function () {
    ...
    $("#btshowConst").bind("click", function () {
        ...
        [{ text: '确定', onclick: f_SaveConstData },
         ...
         }], url: '../../Contract/ProgramInforDetail/ContractConstDetail.aspx?strContractId=' + strContratId + '&strContractCode=' + strContractCode + '&isView=' + isView
        });
    })
    ...
    function SaveConstData(strRequestdata) {
        $.ajax({
            ...
            type: "POST",
            url: "../../Contract/MethodHander.ashx?action=UpdateConstractFeeCount&strContratId=" + strContratId + strRequestdata,
    ...
    $.ajax({
        ...
        async: false, //设置是否为异步加载,此处必须
        type: "POST",
        url: "../../Contract/MethodHander.ashx?action=GetDict&type=RPT_WAY",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            if (data != null) {
    ...
    $("#Rpt_Way").ligerComboBox({ data: vRptItems, width: 200, valueFieldID: 'Rpt_Way_OP', valueField: 'DICT_CODE', textField: 'DICT_TEXT', isMultiSelect: false, initValue: SetRptboxInitValue() });
    ...
    $("#Contract_Date").ligerDateEditor({ width: 200, format: "yyyy-MM-dd", initValue: currentTime() });
...
function CreateDiv() {
    ...
    $.ajax({
        ...
        async: false, //设置是否为异步加载,此处必须
        type: "POST",
        url: "../../Contract/MethodHander.ashx?action=GetContractMonitorType&strContratId=" + strContratId + "",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            if (data != null) {
    ...
    if (vContractMonitorItems.length > 0) {
        ...
        for (var i = 0; i < vContractMonitorItems.length; i++) {
            ...
            if (i == 0) {
                ...
                newDiv += '<iframe frameborder="0" name="showmessage' + MonitorTypeId + '" src= "ContractCheckTab_Since.aspx?strContratId=' + strContratId + '&strMonitorType=' + MonitorTypeId + '&strCompanyIdFrim=' + strCompanyIdFrim + '&strProject=' + strProject + '&strContractTypeId=' + strContratTypeId + '&isView='+ isView +' &strContractCode=' + strContractCode + '"></iframe>';
                newDiv += '</div>';
            }
