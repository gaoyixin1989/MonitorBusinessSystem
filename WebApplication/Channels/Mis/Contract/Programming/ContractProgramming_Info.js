...
//委托书编制 环节 获取委托书 基本信息
var MonitorTypeName = "", ContractTypeName = "", Remarks = "", ConstSum = '0', strConType = "", strConsUrl = "", ConstIdent = "0";
var vMonitorType = null, vContratTypeItem = null, vRemarkItems = null, vConstItems = null, VConType = null;
...
function GetInivalue() {
    ...
    $.ajax({
        ...
        async: false,
        type: "POST",
        url: "../MethodHander.ashx?action=GetDict&type=ConType",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            if (data != null) {
...
//    $("#Test_Type").ligerTextBox({ disabled: true });
    $("#Remarks").ligerTextBox({ disabled: true });
}
function GetIdValueName() {
    strConType = "";
    if (vContractInfor[0].TEST_TYPE != "") {
        for (var i = 0; i < VConType.length; i++) {
            if (vContractInfor[0].TEST_TYPE == VConType[i].DICT_CODE) {
                strConType = VConType[i].DICT_TEXT;
            }
        }
    }
    ContractTypeName = "";
