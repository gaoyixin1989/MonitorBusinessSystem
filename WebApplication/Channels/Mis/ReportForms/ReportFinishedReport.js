...
$(document).ready(function () {
    ...
    function showDetailSrh() {
        if (detailWinSrh) {
            ...
            mainform.ligerForm({
                ...
                fields: [
                     ...
                     { display: "委托类别", name: "SEA_CONTRACT_TYPE", newline: false, type: "select", comboboxName: "SEA_CONTRACT_TYPE_BOX", options: { valueFieldID: "SEA_CONTRACT_TYPE_OP", valueField: "DICT_CODE", textField: "DICT_TEXT", data: vContratTypeItem} },
                     { display: "合同号", name: "SEA_CONTRACT_CODE", newline: true, type: "text" },
                     { display: "执行科室", name: "SEA_DEPT", newline: false, type: "text" },
                     { display: "受检单位", name: "SEA_COMPANY", newline: true, type: "text" }
                    ]
            });

            detailWinSrh = $.ligerDialog.open({
                target: $("#detailSrh"),
                width: 600, height: 240, top: 90, title: "报告完全情况统计报表查询",
                buttons: [
        ...
        function search() {
            ...
            self.location.href = "ReportFinishedReport.aspx?action=GetTaskListData&strYear=" + SEA_YEAR + "&strContractCode=" + SEA_CONTRACT_CODE + "&strMonth=" + SEA_MONTH + "&strQuarter=" + SEA_QUARTER + "&strTestCompany=" + SEA_COMPANYNAME + "&strDept=" + SEA_DEPT +"&strContractType="+SEA_CONTRACT_TYPE;
