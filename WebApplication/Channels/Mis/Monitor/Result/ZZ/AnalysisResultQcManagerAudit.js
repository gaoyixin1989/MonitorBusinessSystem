// Create by 熊卫华 2013.05.09  "质量负责人审核"功能
var objOneGrid = null;
var objTwoGrid = null;
...
var strUrl = "AnalysisResultQcManagerAudit.aspx";
...
$(document).ready(function () {
    objOneGrid = $("#oneGrid").ligerGrid({
        ...
        toolbar: { items: [
                { text: '审核', click: GoToBack, icon: 'add' }, //退回（原来）
                {text: '确认', click: SendToNext, icon: 'add' }, //发送
    ...
    function upLoadFile() {
        ...
        $.ligerDialog.open({ title: '附件上传', width: 500, height: 300, isHidden: false,
            ...
            }], url: '../../../../OA/ATT/AttFileUpload.aspx?id=' + objOneGrid.getSelectedRow().ID
        });
    }
    //下载数据汇总表
                ...
                $.ajax({
                    ...
                    success: function (data, textStatus) {
                        if (data == "1") {
                            ...
                            objFourGrid.set("data", emptyArray);
                            objFiveGrid.set("data", emptyArray);
                            $.ligerDialog.success('任务发送成功！！已发送到【<a style="color:Red;font-weight:bold">技术负责人审核</a>】环节！')
                        }
                        else {
