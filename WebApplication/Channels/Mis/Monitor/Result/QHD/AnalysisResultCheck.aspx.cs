...
/// 创建人  ：熊卫华
/// </summary>
public partial class Channels_Mis_Monitor_Result_QHD_AnalysisResultCheck : PageBase
{
    ...
    private string getOneGridInfo(string iSample)
    {
        ...
        DataTable dt = new TMisMonitorResultLogic().getTaskCheckInfo_QHD(LogInfo.UserInfo.ID, "duty_other_analyse_result", "03,021,02", "2", "40", iSample, intPageIndex, intPageSize);
        int intTotalCount = new TMisMonitorResultLogic().getTaskInfoCheckCount_QHD(LogInfo.UserInfo.ID, "duty_other_analyse_result", "03,021,02", "2", "40", iSample);
    ...
    {
        DataTable dt = new TMisMonitorResultLogic().geResultChecktItemTypeInfoEx_QHD(LogInfo.UserInfo.ID, "duty_other_analyse_result", strOneGridId, "03,021,02", "2", "40");
    ...
    private string getThreeGridInfo(string strTwoGridId, string strMonitorID, string iSample)
    {
        ...
        DataTable dt = new TMisMonitorResultLogic().getTaskSimpleCheckInfo_QHD(strTwoGridId, "2", "40", intPageIndex, intPageSize, iSample, strMonitorID);
        int intTotalCount = new TMisMonitorResultLogic().getTaskSimpleCheckInfoCount_QHD(strTwoGridId, "2", "40", iSample, strMonitorID);
        string strJson = CreateToJson(dt, intTotalCount);
        return strJson;
    }
    /// <summary>
    ...
    private string getFourGridInfo(string strThreeGridId, string strMonitorID, string iSample)
    {
        ...
        //每页记录数
        int intPageSize = Convert.ToInt32(Request.Params["pagesize"]);
        DataTable dt = new TMisMonitorResultLogic().getTaskItemCheckInfo(LogInfo.UserInfo.ID, strThreeGridId, "40", iSample, strMonitorID);
        string strJson = CreateToJson(dt, dt.Rows.Count);
        return strJson;
    }
    /// <summary>
    ...
    {
        bool IsCanGoToBack = new TMisMonitorResultLogic().IsCanSendTaskCheckToNextFlow(strTaskId, LogInfo.UserInfo.ID, "duty_other_analyse_result", "40");
    ...
    {
        bool isSuccess = new TMisMonitorResultLogic().SendResultToNext_QHD(strResultId, "50");
