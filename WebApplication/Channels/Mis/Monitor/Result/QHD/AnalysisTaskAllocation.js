...
$(document).ready(function () {
    ...
    objFourGrid = $("#fourGrid").ligerGrid({
        ...
        height: bottomHeight,
        columns: [
                    { display: '分析项目', name: 'ITEM_ID', align: 'left', width: 200, minWidth: 60, render: function (record) {
                    ...
                    { display: '默认负责人', name: 'USER_DEFAULT', align: 'left', width: 100, minWidth: 60,
                        render: function (record, rowindex, value) {
                            ...
                            if (objUserArray.length > 0)
                                ...
                                strUserName = "请选择";
                            return "<a href=\"javascript:getDefaultUserName('" + record.ID + "','" + record.ITEM_ID + "')\">" + strUserName + "</a> ";
                        }
                    },
                    { display: '分析协同人', name: 'USER_DEFAULT_EX', align: 'left', width: 150, minWidth: 60,
                        render: function (record, rowindex, value) {
                            ...
                            if (objUserArray.length > 0)
                                ...
                                strUserName = "请选择";
                            return "<a href=\"javascript:getDefaultUserExName('" + record.ID + "','" + record.ITEM_ID + "')\">" + strUserName + "</a> ";
                        }
                    },
...
//弹出选择默认负责人
function getDefaultUserName(strResultId, strItemID) {
    var strSampleIDs = "";
    for (var i = 0; i < objThreeGrid.rows.length; i++) {
        strSampleIDs += objThreeGrid.rows[i].ID + ",";
    }
    ...
    $.ligerDialog.open({ title: "选择分析负责人", width: 400, height: 250, isHidden: false, buttons:
        ...
        }], url: "../DefaultUser.aspx?strResultId=" + strResultId + "&strItemID=" + strItemID + "&strSampleIDs=" + strSampleIDs
    });
}
//弹出选择默认协同人
function getDefaultUserExName(strResultId, strItemID) {
    var strSampleIDs = "";
    for (var i = 0; i < objThreeGrid.rows.length; i++) {
        strSampleIDs += objThreeGrid.rows[i].ID + ",";
    }
    $.ligerDialog.open({ title: "选择分析协同人", width: 400, height: 250, isHidden: false, buttons:
        ...
        '关闭', onclick: function (item, dialog) { dialog.close(); }
        }], url: "../DefaultUserEx.aspx?strResultId=" + strResultId + "&strItemID=" + strItemID + "&strSampleIDs=" + strSampleIDs
    });
}
//弹出选择分析完成时间
...
function SendToNextFlow(strTaskId) {
    ...
    $.ajax({
        ...
        async: false,
        type: "POST",
        url: strUrl + "?type=SendToNext&strTaskId=" + strTaskId + "&Sample=" + getQueryString("Sample"),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data, textStatus) {
            isSuccess = data;
