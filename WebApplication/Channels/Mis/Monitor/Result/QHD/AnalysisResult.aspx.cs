...
/// 创建人  ：熊卫华
/// </summary>
public partial class Channels_Mis_Monitor_Result_QHD_AnalysisResult : PageBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            {
                strResult = SendToNext(Request["strSimpleIds"].ToString());
                Response.Write(strResult);
                Response.End();
            }
            //监测结果发送
    ...
    {
        DataTable dt = new TMisMonitorResultLogic().getSimpleCodeInResult_QHD(LogInfo.UserInfo.ID, "20", "2");
        int intTotalCount = new TMisMonitorResultLogic().getSimpleCodeInResultCount_QHD(LogInfo.UserInfo.ID, "20", "2");
    ...
    private string getTwoGridInfo(string strOneGridId)
    {
        ...
        //DataTable dt = new TMisMonitorResultLogic().getResultInResultFlow(strOneGridId, LogInfo.UserInfo.ID, "02", intPageIndex, intPageSize);
        //int intTotalCount = new TMisMonitorResultLogic().getResultInResultFlowCount(strOneGridId, LogInfo.UserInfo.ID, "02");
        DataTable dt = new TMisMonitorResultLogic().getResultInResultFlow_QHD(strOneGridId, LogInfo.UserInfo.ID, "20");
    ...
    /// <returns></returns>
    public string SendToNext(string strSimpleIds)
    {
        string strMsg = new TMisMonitorResultLogic().ResultSendToNext_QHD(strSimpleIds, LogInfo.UserInfo.ID, "20", "30");
        return strMsg == "" ? "[{'result':'1','msg':''}]" : "[{'result':'0','msg':'" + strMsg + "'}]";
    }
    /// <summary>
    /// 样品发送
    /// </summary>
    ...
    [WebMethod]
    public static string FinishAllItem(string strSampleIDs, string strItemID)
    {
        bool isSuccess = new TMisMonitorResultLogic().FinishAllItem(strSampleIDs.TrimEnd(','), strItemID, "1");
        return isSuccess == true ? "1" : "0";
    }
}