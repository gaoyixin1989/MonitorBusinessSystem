...
$(document).ready(function () {
    ...
    objOneGrid = $("#oneGrid").ligerGrid({
        ...
        toolbar: { items: [
                { text: '发送', click: showSend, icon: 'add' },
                  { line: true },
    ...
    function showSend() {
        if (sendhDialog) {
            ...
            $("#sendForm").ligerForm({
                ...
                fields: [
                            { display: "审核人", name: "ddlUser", newline: true, type: "select", comboboxName: "ddlUserBox", options: { valueFieldID: "hidUser", valueField: "ID", textField: "REAL_NAME", resize: false, url: strUrl + "?type=GetCheckUser&strSubTaskID=" + objSubTaskID} }
                        ]
            });
    ...
    function SendToNext() {
        ...
        if (IsCanToSend(objOneGrid.getSelectedRow().ID) == "1") {
            ...
            $.ligerDialog.confirm('您确定将该任务发送至下一环节？', function (yes) {
                if (yes == true) {
                    $.ajax({
                        ...
                        async: false,
                        type: "POST",
                        url: strUrl + "?type=SendToNext&strOneGridId=" + objOneGrid.getSelectedRow().ID + "&strSubTaskID=" + objSubTaskID + "&strUserID=" + $("#hidUser").val(),
                        ...
                        success: function (data, textStatus) {
                            if (data == "1") {
                                ...
                                objThreeGrid.set("data", emptyArray);
                                objFourGrid.set("data", emptyArray);
                                $.ligerDialog.success('任务发送成功!发送至【<a style="color:Red;font-weight:bold">现场室主任审核</a>】环节', '提示', function (result) { if (result) { top.f_removeSelectedTabs(); } })
                            }
                            else {
            ...
            $.ligerDialog.warn('该任务存在分析类现场项目，需要汇总后才能发送');
            return;
        }
    }
        ...
        $.ligerDialog.prompt('退回意见', '', true, function (yes, value) {
            ...
            if (yes) {
                ...
                $.ajax({
                    ...
                    async: false,
                    type: "POST",
                    url: strUrl + "?type=GoToBackTask&strTaskId=" + objOneGrid.getSelectedRow().ID + "&strSubTaskID=" + objSubTaskID + "&strSuggestion=" + encodeURI(value),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (data, textStatus) {
                        if (data == "1") {
...
//发送之前判断是否满足发送条件(如果分析类现场项目还没走到现场复核不能发送)
function IsCanToSend(strTaskId) {
    var isCanBack = false;
    $.ajax({
        cache: false,
        async: false,
        type: "POST",
        url: strUrl + "?type=IsCanToSend&strTaskId=" + strTaskId,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data, textStatus) {
            isCanBack = data;
