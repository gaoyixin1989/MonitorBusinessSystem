// Create by weilin 2014.03.27  现场结果复核列表

var objGridList = null;
var strUrl = "SampleResultCheckList.aspx";
...
function click_done() {
    ...
    if (selected.MONITOR_ID == "000000004" || selected.MONITOR_ID == "FunctionNoise" || selected.MONITOR_ID == "AreaNoise" || selected.MONITOR_ID == "EnvRoadNoise")
        surl = "../Channels/Mis/Monitor/sampling/QY/Sampling_Noise.aspx?Link=Check&strSubtaskID=" + selected.ID + "&strMonitor_ID=" + selected.MONITOR_ID + "&IS_BACK=" + isBack;
    else
        surl = "../Channels/Mis/Monitor/Result/QY/SampleResultCheck.aspx?strSubtaskID=" + selected.ID + "&IS_BACK=" + isBack;
    top.f_addTab(tabid, '现场监测结果复核', surl);
}
