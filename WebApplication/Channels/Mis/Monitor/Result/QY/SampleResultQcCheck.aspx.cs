...
/// 创建人  ：邵世卓
/// </summary>
public partial class Channels_Mis_Monitor_Result_QY_SampleResultQcCheck : PageBase
{
    ...
    private string getOneGridInfo()
    {
        ...
        DataTable dt = new TMisMonitorTaskLogic().SelectSampleTaskForQCQY(strSubTaskID, LogInfo.UserInfo.ID, "sample_result_qccheck", "023", intPageIndex, intPageSize);
        int intTotalCount = new TMisMonitorTaskLogic().SelectSampleTaskCountForQCQY(strSubTaskID, LogInfo.UserInfo.ID, "sample_result_qccheck", "023");
    ...
    private string getTwoGridInfo(string strOneGridId)
    {
        ...
        DataTable dt = new TMisMonitorSampleInfoLogic().GetSampleInfoSourceByTaskForQY_XC(strSubTaskID, strOneGridId, "", intPageIndex, intPageSize);
        int intTotalCount = new TMisMonitorSampleInfoLogic().GetSampleInfoCountByTaskForQY_XC(strOneGridId, "");
        string strJson = CreateToJson(dt, intTotalCount);
        return strJson;
    }
    /// <summary>
    ...
    private string getThreeGridInfo(string strTwoGridId)
    {
        ...
        objResultVo.SAMPLE_ID = strTwoGridId;
        objResultVo.QC_TYPE = "0";
        //objResultVo.RESULT_STATUS = "04";
    ...
    private string SendToNext(string strTaskId)
    {
        ...
        if (IsSuccess == true)
        {
            ...
            if (IsFinish == true)
            {
                ...
                {
                    DataTable objTable = new TMisMonitorSubtaskLogic().SelectByTable(new TMisMonitorSubtaskVo() { TASK_ID = strTaskId });
                    //如果是环境质量将自动对数据进行填报
