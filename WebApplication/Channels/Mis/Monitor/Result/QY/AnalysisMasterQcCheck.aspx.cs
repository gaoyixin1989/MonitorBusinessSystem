...
/// 创建人  ：熊卫华
/// </summary>
public partial class Channels_Mis_Monitor_Result_QY_AnalysisMasterQcCheck : PageBase
{
    ...
    private string getOneGridInfo(string strIsDo)
    {
        ...
        {
            dt = new TMisMonitorResultLogic().getTaskQcCheckInfo_QY(LogInfo.UserInfo.ID, "analysis_result_check", "09", "50", intPageIndex, intPageSize);
            intTotalCount = new TMisMonitorResultLogic().getTaskInfoQcCheckCount_QY(LogInfo.UserInfo.ID, "analysis_result_check", "09", "50");
        }
        //退回意见
    ...
    private string getTwoGridInfo(string strOneGridId, string strIsDo)
    {
        ...
        if (strIsDo == "0")
        {
            dt = new TMisMonitorResultLogic().getTaskSimpleQcCheckInfo(strOneGridId, LogInfo.UserInfo.ID, "analysis_result_check", "03", "50", intPageIndex, intPageSize);
            intTotalCount = new TMisMonitorResultLogic().getTaskSimpleQcCheckInfoCount(strOneGridId, LogInfo.UserInfo.ID, "analysis_result_check", "03", "50");
        }
        else
        {
            dt = new TMisMonitorResultLogic().getTaskSimpleQcCheckInfo(strOneGridId, LogInfo.UserInfo.ID, "analysis_result_check", "09", "50", intPageIndex, intPageSize);
            intTotalCount = new TMisMonitorResultLogic().getTaskSimpleQcCheckInfoCount(strOneGridId, LogInfo.UserInfo.ID, "analysis_result_check", "09", "50");
        }
        string strJson = CreateToJson(dt, intTotalCount);
        return strJson;
    }
    ...
    private string getThreeGridInfo(string strTwoGridId)
    {
        ...
        DataTable dt = new TMisMonitorResultLogic().getTaskItemQcCheckInfo(strTwoGridId, "50");
    ...
    private string GoToBack(string strTaskId, string strSuggestion)
    {
        ...
        isSuccess = new TMisMonitorResultLogic().sendTaskQcCheckInfoToNext_QY(strTaskId, "03", "50", "30", "退回", "2");
    ...
    public string IsCanSendTaskQcToNextFlow(string strTaskId)
    {
        ...
        bool IsCanGoToBack = new TMisMonitorResultLogic().IsCanSendToNext(strTaskId, "50", true);
    ...
    private string SendToNext(string strTaskId)
    {
        ...
        DataTable objTable = new TMisMonitorResultLogic().geResultChecktItemTypeInfo_QHD(LogInfo.UserInfo.ID, "analysis_result_qccheck", strTaskId, "03", "0", "50");
        ...
        //判断该任务是否有外控样，如果有的话发送给质控审核(40),如果没有发送给分析室主任审核(40)
        bool isHasOuterQcSample = new TMisMonitorResultLogic().CheckTaskHasOuterQcSample(strTaskId, "");
        if (isHasOuterQcSample == true)
        {
            IsSuccess = new TMisMonitorResultLogic().sendTaskQcCheckInfoToNext_QY(strTaskId, "03", "50", "40", "发送", "2");
            strMsg = "质控审核";
        }
        else
        {
    ...
    public string ResultGoToBack(string strResultId, string strSuggestion)
    {
        ...
        TMisMonitorResultVo.RESULT_STATUS = "30";
