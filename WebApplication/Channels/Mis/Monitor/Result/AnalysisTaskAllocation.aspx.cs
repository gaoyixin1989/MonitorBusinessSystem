...
/// 创建人  ：熊卫华
/// </summary>
public partial class Channels_Mis_Monitor_Result_AnalysisTaskAllocation : PageBase
{
    ...
    /// 获取分配情况信息
    /// </summary>
    /// <param name="strOneGridId">任务ID</param>
    /// <returns></returns>
    private string getFourGridInfo(string strOneGridId)
    {
        DataTable dt = new TMisMonitorResultLogic().getAssignedDefaultUser(strOneGridId);
    ...
    private string getThreeGridInfo(string strTwoGridId)
    {
        ...
        DataTable dt = new TMisMonitorResultLogic().getItemInfo(strTwoGridId, "01", intPageIndex, intPageSize);
        int intTotalCount = new TMisMonitorResultLogic().getItemInfoCount(strTwoGridId, "01");
        string strJson = CreateToJson(dt, intTotalCount);
        return strJson;
    }
    /// <summary>
