...
function SendClick() {
    var objPGrid = $("#SamplePointID")[0].contentWindow.objPointGrid;
    var objRGrid = $("#SampleResultID")[0].contentWindow.objOneGrid;

    if (objPGrid.getSelectedRows().length == 0) {
        $.ligerDialog.warn('请选择监测点位进行发送');
        return;
    }
    var strSampleIDs = "";
    for (var i = 0; i < objPGrid.getSelectedRows().length; i++) {
        strSampleIDs += objPGrid.getSelectedRows()[i].ID + ",";
    }
    
    $.ligerDialog.confirm('您确定要将该任务发送至下一环节吗？', function (yes) {
        if (yes == true) {
            var fn = $("#SampleLicaleID")[0].contentWindow.getSaveDate();
            var strData = "{'strSubtaskID:'" + $("#SUBTASK_ID").val() + "','strSampleIDs':'" + strSampleIDs + "'" + fn + "}";
            $.ajax({
                cache: false,
                async: false,
                type: "POST",
                url: strUrl + "/btnSendClick",
                data: "{'strSubtaskID':'" + $("#SUBTASK_ID").val() + "','strSampleIDs':'" + strSampleIDs + "'" + fn + "}",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data, textStatus) {
                    strValue = data.d;
                    if (strValue == "1") {
                        ...
                        $.ligerDialog.confirmWF('发送成功', '提示', function (result) { if (result) { top.f_removeSelectedTabs(); } })
                    }
                    else {
