...
/// 创建人  ：熊卫华
/// </summary>
public partial class Channels_Mis_Monitor_sampling_QHD_QcTwinSetting : PageBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request["iSampleCode"] != null && Request["iSampleCode"].ToString() != "")
            this.iSampleCode.Value = Request["iSampleCode"].ToString();
    }
    /// <summary>
    /// 获取样品-监测项目
    /// </summary>
    ...
    /// <returns></returns>
    [WebMethod]
    public static bool QcSave(string strSampleID, string strQcType, string strItemId, string strQcCount, string iSampleCode)
    {
        bool isSuccess = true;
        deleteSampleInfo(strSampleID, strQcType);
        if (strQcCount == "1")
            createQcInfo(strSampleID, strQcType, strItemId, iSampleCode);
        if (strQcCount == "2")
        {
            createQcInfo(strSampleID, strQcType, strItemId, iSampleCode);
            createQcInfo(strSampleID, strQcType, strItemId, iSampleCode);
