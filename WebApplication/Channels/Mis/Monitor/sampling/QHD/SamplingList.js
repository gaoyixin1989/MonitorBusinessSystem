...
function click_OfDblclick(data) {
    ...
    var surl = "../Channels/Mis/Monitor/sampling/QHD/Sampling.aspx?strSubtaskID=" + data.ID + "&strMonitorID=" + data.MONITOR_ID;
    ...
    switch (item.id) {
        case 'done':
            ...
            if (!selected) { $.ligerDialog.warn('请先选择一条记录！'); return }
                ...
                var tabid = "tabidSamplingNew"
                var surl = "../Channels/Mis/Monitor/sampling/QHD/Sampling.aspx?strTaskID=" + selected.TASK_ID + "&strSubtaskID=" + selected.ID + "&strMonitorID=" + selected.MONITOR_ID;
                top.f_addTab(tabid, '采样', surl);
            }
