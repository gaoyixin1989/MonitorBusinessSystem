...
/// 创建人  ：苏成斌
/// </summary>
public partial class Channels_Mis_Monitor_sampling_SamplingTaskAllocation_QCStep : PageBase
{
    ...
    /// </summary>
    /// <returns></returns>
    /// 修改时间20140509 huangjinun 添加了一个参数‘strALLQCSTATUS’
    [WebMethod]
    public static string btnSendClick(string strPlanID, string strSampleAskDate, string strSampleFinishDate, string strTicketNum, string strQCStatus, string strALLQCSTATUS)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            if (dtApp.Rows.Count > 0)
            {
                ...
                objSubApp.QC_USER_ID = new PageBase().LogInfo.UserInfo.ID;
                objSubApp.QC_DATE = DateTime.Now.ToString();
