...
function SendClick() {
    $.ligerDialog.confirm('您确定要将该任务发送至下一环节吗？', function (yes) {
        if (yes == true) {
            ...
            var strData = "{'strSubtaskID:'" + $("#SUBTASK_ID").val() + "'" + fn + "}";
            $.ajax({
                cache: false,
                async: false,
                type: "POST",
...
//采样任务退回
function BackClick() {
    $.ligerDialog.confirm('您确定要将该任务回退至上一环节吗？', function (yes) {
        if (yes == true) {
            $.ajax({
                cache: false,
                async: false,
                ...
                data: "{'strSubtaskID':'" + $("#txtSubTaskId").val() + "'}",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data, textStatus) {
                    strValue = data.d;
                    if (strValue == "1") {
                        $.ligerDialog.confirmWF('退回成功！已退回到【<a style="color:Red;font-weight:bold">采样任务分配</a>】环节！', '提示', function (result) { if (result) { top.f_removeSelectedTabs(); } })
                    }
                    else {
