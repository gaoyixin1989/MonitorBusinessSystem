...
$(document).ready(function () {
    ...
    objGrid = $("#grid").ligerGrid({
        ...
        toolbar: { items: [
                ...
                { text: '计算', click: Calculate, icon: 'database_wrench' },
            ...
            success: function (json) {
                if (parseInt(json.Total) > 0) {
                    ...
                    $.each(json.UnSureColumns, function (i, n) {
                        //相应可以编辑的列做相应的处理 
                        if (n.columnId.indexOf("_ITEM") != -1 || n.columnId.indexOf("DAY") != -1 || n.columnId.indexOf("Is_Run") != -1 || n.columnId.indexOf("Load_Mode") != -1 || n.columnId.indexOf("In_Water_Qty") != -1 || n.columnId.indexOf("Is_Evaluate") != -1 || n.columnId.indexOf("WaterQty") != -1) { 
                            columnsArr.push({ display: n.columnName, name: n.columnId, width: parseInt(n.columnWidth) + 80, minWidth: 60, align: "center", editor: { type: "text"} });
                        }
                        else {
...
function Calculate() {
    var rowcount = objGrid.data.Total;
    var rows = objGrid.data.Rows;
    for (var i = 0; i < rowcount; i++) {
        $.ajax({
            cache: false,
            async: false,
            type: "POST",
            url: url + "/CalculateData",
            data: "{'strId':'" + rows[i].ID + "'}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
...
function ExcelImport() {
    $.ligerDialog.open({ title: "Excel导入界面", width: 500, height: 200, isHidden: false, buttons:
        ...
        '确定', onclick: function (item, dialog) {
            value = $("iframe")[0].contentWindow.Import("PolluteWater");
        }
        }, { text:
...
function ExcelExport() {
    ...
    var year = $("#hidYear").val();
    if (month != "" && year != "") {
        var vsURL = "../FillExportDemo.aspx?action=FillData&type=PolluteWater&month=" + month + "&year=" + year;
