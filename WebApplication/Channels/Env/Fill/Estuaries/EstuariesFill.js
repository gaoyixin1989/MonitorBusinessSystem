// Create by 钟杰华 2013-02-22  "入海河口数据填报"功能

var objGrid = null;
var gridJSON = null;
var url = "EstuariesFill.aspx";
            ...
            if (obj.ID.length > 0) {
                if (obj.STATUS == "" || obj.STATUS == "0") {
                    $.ligerDialog.confirm('确定填报数据已经完整？', function (yes) {
                        if (yes) {
                            var surl = '../Sys/WF/WFStartPage.aspx?action=|type=true|pf_id=ET^' + year + '^' + month + '|&WF_ID=PF';
...
function showDataImport() {
    $.ligerDialog.open({ title: "Excel导入界面", width: 500, height: 200, isHidden: false, buttons: 
        ...
        '确定', onclick: function (item, dialog) {
            value = $("iframe")[0].contentWindow.Import("Estuaries"); 
        }
        }, { text:
...
function showDataExport() {
    if (objGrid.rows.length > 0) {
        ...
        var year = $("#hidYear").val();
        if (month != "" && year != "") {
            var vsURL = "../FillExportDemo.aspx?action=FillData&type=Estuaries&month=" + month + "&year=" + year; 
