// Modify by 刘静楠 2013.06.24  "环境空气(小时)数据填报"功能

var objGrid = null;
var url = "AirHourFill.aspx"; 
            ...
            success: function (json) {
                if (parseInt(json.Total) > 0) {
                    ...
                    $.each(json.UnSureColumns, function (i, n) {
                        //相应可以编辑的列做相应的处理
                        if (n.columnId.indexOf("T_ENV_FILL_AIRHOUR_ITEM") != -1) {
            ...
            if (obj.ID.length > 0) {
                if (obj.STATUS == "" || obj.STATUS == "0") {
                    $.ligerDialog.confirm('确定填报数据已经完整？', function (yes) {
                        if (yes) {
                            var surl = '../Sys/WF/WFStartPage.aspx?action=|type=true|pf_id=AH^' + year + '^' + month + '|&WF_ID=PF';
...
function showDataImport() {
    $.ligerDialog.open({ title: "Excel导入界面", width: 500, height: 200, isHidden: false, buttons:
        ...
        '确定', onclick: function (item, dialog) {
            value = $("iframe")[0].contentWindow.Import("AirHour"); 
        }
        }, { text:
...
function showDataExport() {
    if (objGrid.rows.length > 0) {
        ...
        var year = $("#hidYear").val();
        if (month != "" && year != "") {
            var vsURL = "../FillExportDemo.aspx?action=FillData&type=AirHour&month=" + month + "&year=" + year; 
