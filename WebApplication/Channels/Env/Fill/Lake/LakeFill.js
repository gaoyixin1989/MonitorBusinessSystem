// Create by 魏林 2013.06.22  "湖库数据填报"功能

var objGrid = null;
var gridJSON = null;
var url = "LakeFill.aspx";
            ...
            if (obj.ID.length > 0) {
                if (obj.STATUS == "" || obj.STATUS == "0") {
                    $.ligerDialog.confirm('确定填报数据已经完整？', function (yes) {
                        if (yes) {
                            var surl = '../Sys/WF/WFStartPage.aspx?action=|type=true|pf_id=HK^' + year + '^' + month + '|&WF_ID=PF';
...
function ExcelImport() {
    $.ligerDialog.open({ title: "Excel导入界面", width: 500, height: 200, isHidden: false, buttons:
        ...
        '确定', onclick: function (item, dialog) {
            value = $("iframe")[0].contentWindow.Import("Lake");
        }
        }, { text:
...
function ExcelExport() {
    ...
    var year = $("#hidYear").val();
    if (month != "" && year != "") {
        var vsURL = "../FillExportDemo.aspx?action=FillData&type=Lake&month=" + month + "&year=" + year;
