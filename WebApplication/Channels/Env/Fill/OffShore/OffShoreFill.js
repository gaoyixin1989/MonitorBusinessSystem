// Create by 钟杰华 2013.02.23  "近岸直排数据填报"功能 ,Modify by 刘静楠

var objGrid = null;
var gridJSON = null;
var url = "OffShoreFill.aspx";
...
$(document).ready(function () {
    ...
    $("#searchForm").ligerForm({
        ...
        fields: [
                      ...
                      { display: "监测点", name: "ddlPoint", newline: true, type: "select", comboboxName: "ddlPointBox", options: { valueFieldID: "hidPoint", valueField: "ID", textField: "COMPANY_NAME", resize: false} }
                    ]
    });
    //构建流程表单
    $("#flowForm").ligerForm({
            ...
            success: function (json) {
                if (parseInt(json.Total) > 0) {
                    ...
                    $.each(json.UnSureColumns, function (i, n) {
                        //相应可以编辑的列做相应的处理
                        if (n.columnId.indexOf("ITEM") != -1 || n.columnId.indexOf("DAY") != -1 || n.columnId.indexOf("COMPANY_NAME") != -1 || n.columnId.indexOf("POINT_CODE") != -1 || n.columnId.indexOf("POINT_NAME") != -1 || n.columnId.indexOf("JUDGE") != -1 || n.columnId.indexOf("OVERPROOF") != -1||n.columnId.indexOf("FLUX")!=-1 || n.columnId.indexOf("LET_TIME") != -1 || n.columnId.indexOf("WATER_TOTAL") != -1) {
                            columnsArr.push({ display: n.columnName, name: n.columnId, width: parseInt(n.columnWidth), minWidth: 140, align: "center", editor: { type: "text"} });
                        }
                        else {
                    ...
                    objGrid.toggleCol("T_ENV_FILL_OFFSHORE@SAMPLING_DAY@采样日期");
            ...
            if (obj.ID.length > 0) {
                if (obj.STATUS == "" || obj.STATUS == "0") {
                    $.ligerDialog.confirm('确定填报数据已经完整？', function (yes) {
                        if (yes) {
                            var surl = '../Sys/WF/WFStartPage.aspx?action=|type=true|pf_id=OF^' + year + '^' + month + '|&WF_ID=PF';
...
function setSamplingDay() {
    ...
    for (var i = 0; i < gridJSON.Rows.length; i++) {
      ...
      //  objGrid.updateCell("sampling_day", day, rowDom);
    }
}
...
function showDataImport() {
    $.ligerDialog.open({ title: "Excel导入界面", width: 500, height: 200, isHidden: false, buttons: 
        ...
        '确定', onclick: function (item, dialog) {
            value = $("iframe")[0].contentWindow.Import("OffShore");
        }
        }, { text:
...
function showDataExport() {
    if (objGrid.rows.length > 0) {
        ...
        var year = $("#hidYear").val();
        if (month != "" && year != "") {
            var vsURL = "../FillExportDemo.aspx?action=FillData&type=OffShore&month=" + month + "&year=" + year; 
