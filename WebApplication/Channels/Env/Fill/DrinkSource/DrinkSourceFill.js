// Create By 魏林 2013-06-24  饮用水源地数据填报管理

var objGrid = null;
var gridJSON = null;
var url = "DrinkSourceFill.aspx";
                ...
                if (parseInt(json.Total) > 0) {
                    ...
                    $.each(json.UnSureColumns, function (i, n) {
                        ...
                        if (n.columnId.indexOf("_ITEM") != -1 || n.columnId.indexOf("DAY") != -1 || n.columnId.indexOf("OVERPROOF") != -1) {
                            columnsArr.push({ display: n.columnName, name: n.columnId, width: parseInt(n.columnWidth)+20, minWidth: 180, align: "center", editor: { type: "text"} });
                        }
                        else {
                            if (n.columnId.indexOf("KPF") != -1) {
                                columnsArr.push({ display: n.columnName, name: n.columnId, width: parseInt(n.columnWidth), minWidth: 180, align: "center", editor: { type: "select", valueField: "DICT_CODE", textField: "DICT_TEXT", data: KPFJson() },
                                ...
                                if (n.columnId.indexOf("SECTION_ID") != -1) {
                                    columnsArr.push({ display: n.columnName, name: n.columnId, width: parseInt(n.columnWidth), minWidth: 180, align: "center",
                                        render: function (item, i, v) {
                                    ...
                                    if (n.columnId.indexOf("POINT_ID") != -1) {
                                        columnsArr.push({ display: n.columnName, name: n.columnId, width: parseInt(n.columnWidth), minWidth: 200, align: "center",
                                            render: function (item, i, v) {
            ...
            if (obj.ID.length > 0) {
                if (obj.STATUS == "" || obj.STATUS == "0") {
                    $.ligerDialog.confirm('确定填报数据已经完整？', function (yes) {
                        if (yes) {
                            var surl = '../Sys/WF/WFStartPage.aspx?action=|type=true|pf_id=DS^' + year + '^' + month + '|&WF_ID=PF';
...
function ExcelImport() {
    $.ligerDialog.open({ title: "Excel导入界面", width: 500, height: 200, isHidden: false, buttons:
        ...
        '确定', onclick: function (item, dialog) {
            value = $("iframe")[0].contentWindow.Import("DrinkSource");
        }
        }, { text:
...
function ExcelExport() {
    ...
    var year = $("#hidYear").val();
    if (month != "" && year != "") {
        var vsURL = "../FillExportDemo.aspx?action=FillData&type=DrinkSource&month=" + month + "&year=" + year;
...
function upLoadFile() {
    ...
    if (ID != null) {
        $.ligerDialog.open({ title: '附件上传', width: 800, height: 350, isHidden: false,
            ...
            }], url: '../FillAttachment/FillAttachmentaspx.aspx?filetype=DrinkSource&Save=SaveFWData&IsInsert=false&ID=' + ID + "First"
        });
    }
}
