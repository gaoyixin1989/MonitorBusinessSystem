// Create by 潘德军 2013.5.9  "双三十水质数据填报"功能

var objGrid = null;
var gridJSON = null;
var url = "River30Fill.aspx";
    ...
    $(document).ready(function () {
        ...
        $("#searchForm").ligerForm({
            ...
            fields: [
                    ...
                    //  { display: "所在地区", name: "ddlArea", newline: true, type: "select", comboboxName: "ddlAreaBox", options: { valueFieldID: "hidArea", valueField: "DICT_CODE", textField: "DICT_TEXT", url: url + "?action=getDict&dictType=dict_30Area"} }, 
        ...
        //查询下拉框事件
        $.ligerui.get("ddlMonthBox").bind("selected", function () { $("#hidMonthToSave").val($("#hidMonth").val()); $("#hidMonthToExport").val($("#hidMonth").val()); });
...
//                { text: '数据导入', click: function () { showDataImport("EVN_RIVER", getData) }, icon: 'database_wrench' },
//                { text: '数据导出', click: function () { showDataExport("EVN_RIVER") }, icon: 'excel' }
                ]
            },
                ...
                success: function (json) {
                    if (parseInt(json.Total) > 0) {
                        ...
                        $.each(json.UnSureColumns, function (i, n) {
                            ...
                            //相应可以编辑的列做相应的处理
                            if (n.columnId.indexOf("_ITEM") != -1 || n.columnId.indexOf("DAY") != -1 || n.columnId.indexOf("HOUR") != -1 || n.columnId.indexOf("MINUTE") != -1 || n.columnId.indexOf("KPF") != -1 || n.columnId.indexOf("WATER_TYPE_CODE") != -1 || n.columnId.indexOf("OVERPROOF") != -1) {
    ...
    function f_onAfterEdit(e) {
        ...
        if (e.record["__status"] != "nochanged") {
            ...
            $.ajax({
                url: url,
                data: "action=SaveData&id=" + ID + "&updateName=" + updateName + "&value=" + value,
                type: "post",
                dataType: "json",
                async: true,
                cache: false,
                ...
                if (obj.ID.length > 0) {
                    if (obj.STATUS == "" || obj.STATUS == "0") {
                        $.ligerDialog.confirm('确定填报数据已经完整？', function (yes) {
                            if (yes) {
                                var surl = '../Sys/WF/WFStartPage.aspx?action=|type=true|pf_id=RR^' + year + '^' + month + '|&WF_ID=PF';
