...
// Create By 魏林 2013-06-18 "生态补偿数据填报"功能

var objGrid = null;
var gridJSON = null;
var url = "PayforFill.aspx";
            ...
            success: function (json) {
                if (parseInt(json.Total) > 0) {
                    ...
                    $.each(json.UnSureColumns, function (i, n) {
                        //相应可以编辑的列做相应的处理
                        if (n.columnId.indexOf("_ITEM") != -1 || n.columnId.indexOf("DAY") != -1 || n.columnId.indexOf("UP_") != -1) {
            ...
            if (obj.ID.length > 0) {
                if (obj.STATUS == "" || obj.STATUS == "0") {
                    $.ligerDialog.confirm('确定填报数据已经完整？', function (yes) {
                        if (yes) {
                            var surl = '../Sys/WF/WFStartPage.aspx?action=|type=true|pf_id=PF^' + year + '^' + month + '|&WF_ID=PF';
...
function upLoadFile() {
    ...
    if (ID != null) {
        $.ligerDialog.open({ title: '附件上传', width: 800, height: 350, isHidden: false,
            ...
            }], url: '../FillAttachment/FillAttachmentaspx.aspx?filetype=Payfor&Save=SaveFWData&IsInsert=false&ID=' + ID + "First"
        });
    }
}
