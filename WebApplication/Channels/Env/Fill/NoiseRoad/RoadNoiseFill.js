...
// Create By 魏林 2013-06-27 "道路交通噪声数据填报"功能
...
$(document).ready(function () {
    ...
    objSummaryGrid = $("#gridSummary").ligerGrid({
        ...
        columns: [
                ...
                { display: '道路交通噪声强度等级', name: 'REMARK2', width: 160, minWidth: 100, align: "center", editor: { type: "text"} }
                ],
        toolbar: { items: [
            ...
            if (obj.ID.length > 0) {
                if (obj.STATUS == "" || obj.STATUS == "0") {
                    $.ligerDialog.confirm('确定填报数据已经完整？', function (yes) {
                        if (yes) {
                            var surl = '../Sys/WF/WFStartPage.aspx?action=|type=true|pf_id=NR^' + year + '^' + month + '|&WF_ID=PF';
...
function ExcelExport() {
    ...
    var year = $("#hidYear").val();
    if (month != "" && year != "") {
        var vsURL = "../FillExportDemo.aspx?action=FillData&type=NoiseRoad&month=" + month + "&year=" + year;
