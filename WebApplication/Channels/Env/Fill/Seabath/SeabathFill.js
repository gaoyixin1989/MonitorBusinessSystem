// Create by 刘静楠 2013.06.25  "海水浴场数据填报"功能
var objGrid = null;
var gridJSON = null;
var url = "SeabathFill.aspx";
    ...
    objGrid = $("#grid").ligerGrid({
        ...
        toolbar: { items: [
             ...
             //{ text: '保存', click: saveData, icon: 'save' },
                {text: '数据导入', click: function () { showDataImport("EVN_Seabath", getData) }, icon: 'database_wrench' },
                { text: '数据导出', click: function () { showDataExport("EVN_Seabath") }, icon: 'excel' },
        ...
        $.ajax({
            ...
            success: function (json) {
                if (parseInt(json.Total) > 0) {
                    gridJSON = json;
                    var columnsArr = [];            //构建表格固定的列
                    //动态的列
                    $.each(json.UnSureColumns, function (i, n) {
                        //相应可以编辑的列做相应的处理
                        if (n.columnId.indexOf("ITEM") != -1 || n.columnId.indexOf("DAY") != -1 || n.columnId.indexOf("HOUR") != -1 || n.columnId.indexOf("MINUTE") != -1 || n.columnId.indexOf("MONTH") != -1 || n.columnId.indexOf("JUDGE") != -1 || n.columnId.indexOf("OVERPROOF") != -1 || n.columnId.indexOf("SAMPLING_DAY") != -1) {
                    ...
                    objGrid.toggleCol("T_ENV_FILL_SEABATH@SAMPLING_DAY@采样日期"); //采样日期
                    $(".l-grid-hd-cell-btn-checkbox").css("display", "none"); //去除第一行第一列的复选框
                }
            }
        });
            ...
            if (obj.ID.length > 0) {
                if (obj.STATUS == "" || obj.STATUS == "0") {
                    $.ligerDialog.confirm('确定填报数据已经完整？', function (yes) {
                        if (yes) {
                            var surl = '../Sys/WF/WFStartPage.aspx?action=|type=true|pf_id=SB^' + year + '^' + month + '|&WF_ID=PF';
