// Create by 熊卫华 2012.11.20  "近岸直排监测点管理"功能  Modify by 刘静楠 
...
var strTableName = "T_ENV_P_OFFSHORE_ITEM";
var strColumnName = "ITEM_ID";
var strWhereColumnName = "POINT_ID"; 

var strOneGridTitle = "近岸直排监测点管理";

var strOneGridAddName = "近岸直排监测点信息增加";
var strOneGridUpdateName = "近岸直排监测点信息编辑";
 
var strUrl = "OffshoreList.aspx";
var strOneGridUpdateUrl = "OffshoreEdit.aspx";
...
$(document).ready(function () {
    ...
    objOneGrid = $("#oneGrid").ligerGrid({
        ...
        columns: [
                ...
                { display: '排污口代码', name: 'POINT_CODE', width: 100, minWidth: 60 },
                { display: '排污口', name: 'POINT_NAME', width: 100, minWidth: 60 }, //NAME的值要和sql中表的字段一致
                {display: '企业名称', name: 'COMPANY_NAME', minWidth: 120 },
                { display: '具体位置', name: 'LOCATION', minWidth: 350 }
                ],
        toolbar: { items: [
        ...
        '确定', onclick: function (item, dialog) {
            ...
            if (v) {
                ...
                $.ajax({
                    ...
                    data: "{'strTable':'T_ENV_P_OFFSHORE','strYear':'" + Year_To + "', 'strMonth':'" + Month_To + "'}",
                    ...
                    success: function (data, textStatus) {
                        if (data.d == "1") {
                            ...
                            function (yes) {
                                if (yes == true) {
                                    var rText = $("iframe")[0].contentWindow.DataCopy("type=copyOffshoreList&Year_From=" + Year_From + "&Month_From=" + Month_From + "&Year_To=" + Year_To + "&Month_To=" + Month_To);
                                    var d = eval(rText);
                                    if (d.result) {
                                        dialog.close();
        ...
        if (detailWinSrh) {
            ...
            divmainform.ligerForm({
                ...
                fields: [
                        ...
                        { display: "企业名称", name: "SEA_COMPANYNAME", newline: true, type: "text" },
                        { display: "具体位置", name: "SEA_LOCATION", newline: true, type: "text" }
                        ]
            });

            detailWinSrh = $.ligerDialog.open({
                target: $("#Seachdetail"),
                width: 350, height: 200, top: 90, title: strOneGridTitle + "查询",
                buttons: [
    ...
    function search() {
        ...
        var SEA_LOCATION = encodeURI($("#SEA_LOCATION").val());
        objOneGrid.set('url', "" + strUrl + "?type=getOneGridInfo&srhYear= " + SEA_YEAR + "&srhName=" + SEA_COMPANYNAME + "&srhLocation=" + SEA_LOCATION);
    }
...
$(document).ready(function () {
    objTwoGrid = $("#twoGrid").ligerGrid({
        ...
        width: '100%',
        pageSizeOptions: [5, 10, 15, 20],
        height: gridHeight, heightDiff: -20, 
        columns: [
    ...
    function CopyV() {
        if (strOneGridId == "") {
            $.ligerDialog.warn('请先选择近岸直排监测点信息');
            return;
        }
    ...
    function PasteItem() {
        ...
        $.ajax({
            ...
            async: true,
            type: "POST",
            url: "../PointInfoCopy.aspx?type=CopyOffshoreEditer&fromID=" + $("#VerticalID").val() + "&toID=" + strOneGridId,
            contentType: "application/json; charset=utf-8",
            dataType: "text",
            success: function (data, textStatus) {
                var d = eval(data);
