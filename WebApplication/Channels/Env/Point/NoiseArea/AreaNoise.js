// Create by 魏林 2013.06.17  "区域环境噪声监测点管理"功能
...
var strTableName = "T_ENV_P_NOISE_AREA_ITEM";
var strColumnName = "ITEM_ID";
var strWhereColumnName = "POINT_ID";

var strOneGridTitle = "区域环境噪声监测点信息";

var strOneGridAddName = "区域环境噪声监测点信息增加";
var strOneGridUpdateName = "区域环境噪声监测点信息编辑";

var strUrl = "AreaNoise.aspx";
var strOneGridUpdateUrl = "AreaNoiseEdit.aspx";
...
$(document).ready(function () {
    ...
    objOneGrid = $("#oneGrid").ligerGrid({
        ...
        columns: [
                ...
                { display: '功能区', name: 'FUNCTION_AREA_ID', render: function (record) {
                    return getDictName(record.FUNCTION_AREA_ID, 'Function_Area');
                }
                },
                { display: '声源类型', name: 'SOUND_SOURCE_ID', render: function (record) {
                    return getDictName(record.SOUND_SOURCE_ID, 'NoiseSource_Type');
                }
                }
                ],
        toolbar: { items: [
        ...
        '确定', onclick: function (item, dialog) {
            ...
            if (v) {
                ...
                $.ajax({
                    ...
                    data: "{'strTable':'T_ENV_P_NOISE_AREA','strYear':'" + Year_To + "', 'strMonth':'" + Month_To + "'}",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (data, textStatus) {
                        if (data.d == "1") {
                            ...
                            function (yes) {
                                if (yes == true) {
                                    var rText = $("iframe")[0].contentWindow.DataCopy("type=copyAreaNoise&Year_From=" + Year_From + "&Month_From=" + Month_From + "&Year_To=" + Year_To + "&Month_To=" + Month_To);
                                    var d = eval(rText);
                                    if (d.result) {
                                        dialog.close();
        ...
        if (detailWinSrh) {
            ...
            divmainform.ligerForm({
                ...
                fields: [
                        ...
                        { display: "行政区", name: "AREA_NAME", newline: true, type: "select", comboboxName: "AREA_NAME_BOX", options: { valueFieldID: "AREA_NAME_OP", valueField: "DICT_CODE", textField: "DICT_TEXT", url: strOneGridUpdateUrl + "?type=getDict&dictType=administrative_area"} },
                        { display: "功能区", name: "FUN_NAME", newline: true, type: "select", comboboxName: "FUN_NAME_BOX", options: { valueFieldID: "FUN_NAME_OP", valueField: "DICT_CODE", textField: "DICT_TEXT", url: strOneGridUpdateUrl + "?type=getDict&dictType=Function_Area"} }
                        ]
            });

            detailWinSrh = $.ligerDialog.open({
                target: $("#Seachdetail"),
                width: 400, height: 260, top: 90, title: strOneGridTitle + "查询",
                buttons: [
    ...
    function search() {
        ...
        var SEA_FUN = encodeURI($("#FUN_NAME_OP").val());

        objOneGrid.set('url', "" + strUrl + "?type=getOneGridInfo&srhYear= " + SEA_YEAR + "&srhMonth= " + SEA_MONTH + "&srhName=" + SEA_NAME + "&srhArea=" + SEA_AREA + "&srhFun=" + SEA_FUN);
    ...
    function clearSearchDialogValue() {
        ...
        $("#FUN_NAME_BOX").ligerGetComboBoxManager().setValue("");
    }
...
$(document).ready(function () {
    ...
    function PasteItem() {
        ...
        $.ajax({
            ...
            async: true,
            type: "POST",
            url: "../PointInfoCopy.aspx?type=copyAreaNoiseV&fromID=" + $("#VerticalID").val() + "&toID=" + strOneGridId,
            //data: "{}",
            contentType: "application/json; charset=utf-8",
            dataType: "text",
            success: function (data, textStatus) {
