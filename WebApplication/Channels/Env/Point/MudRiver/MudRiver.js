//Create By 魏林 2013.06.15 "沉积物（河流）监测点管理"功能
...
var strTableName = "T_ENV_P_MUD_RIVER_V_ITEM";
var strColumnName = "ITEM_ID";
var strWhereColumnName = "POINT_ID";

var strOneGridTitle = "沉积物（河流）断面信息";

var strOneGridAddName = "沉积物（河流）断面信息增加";
var strOneGridUpdateName = "沉积物（河流）断面信息编辑";

var strUrl = "MudRiver.aspx";
var strOneGridUpdateUrl = "MudRiverEdit.aspx";
...
var strTwoGridUpdateUrl = "MudRiverVerticalEdit.aspx";
        ...
        '确定', onclick: function (item, dialog) {
            ...
            if (v) {
                ...
                $.ajax({
                    ...
                    data: "{'strTable':'T_ENV_P_MUD_RIVER','strYear':'" + Year_To + "', 'strMonth':'" + Month_To + "'}",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (data, textStatus) {
                        if (data.d == "1") {
                            ...
                            function (yes) {
                                if (yes == true) {
                                    var rText = $("iframe")[0].contentWindow.DataCopy("type=copyMudRiver&Year_From=" + Year_From + "&Month_From=" + Month_From + "&Year_To=" + Year_To + "&Month_To=" + Month_To);
                                    var d = eval(rText);
                                    if (d.result) {
                                        dialog.close();
            ...
            detailWinSrh = $.ligerDialog.open({
                ...
                buttons: [
                  ...
                  { text: '取消', onclick: function (item, dialog) {
                      clearSearchDialogValue(); detailWinSrh.hide();
                  } 
                  }
                  ]
            });
...
$(document).ready(function () {
    ...
    function PasteItem() {
        ...
        $.ajax({
            ...
            async: true,
            type: "POST",
            url: "../PointInfoCopy.aspx?type=copyMudRiverV&fromID=" + $("#VerticalID").val() + "&toID=" + strTwoGridId,
            //data: "{}",
            contentType: "application/json; charset=utf-8",
            dataType: "text",
            success: function (data, textStatus) {
