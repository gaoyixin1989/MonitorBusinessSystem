// Create by 熊卫华 2012.11.19  "降水监测点管理"功能  Modify by 刘静楠 
...
var strTableName = "T_ENV_P_RAIN_ITEM";
var strColumnName = "ITEM_ID";
var strWhereColumnName = "POINT_ID"; 

var strOneGridTitle = "降雨监测点信息";

var strOneGridAddName = "降雨监测点信息增加";
var strOneGridUpdateName = "降雨监测点信息编辑";

var strUrl = "RainList.aspx";
var strOneGridUpdateUrl = "RainEdit.aspx";
        ...
        '确定', onclick: function (item, dialog) {
            ...
            if (v) {
                ...
                $.ajax({
                    ...
                    data: "{'strTable':'T_ENV_P_RAIN','strYear':'" + Year_To + "', 'strMonth':'" + Month_To + "'}",
                    ...
                    success: function (data, textStatus) {
                        if (data.d == "1") {
                            ...
                            function (yes) {
                                if (yes == true) {
                                    var rText = $("iframe")[0].contentWindow.DataCopy("type=copyRain&Year_From=" + Year_From + "&Month_From=" + Month_From + "&Year_To=" + Year_To + "&Month_To=" + Month_To);
                                    var d = eval(rText);
                                    if (d.result) {
                                        dialog.close();
...
$(document).ready(function () {
    ...
    function PasteItem() {
        ...
        $.ajax({
            ...
            async: true,
            type: "POST",
            url: "../PointInfoCopy.aspx?type=CopyRain_Item&fromID=" + $("#VerticalID").val() + "&toID=" + strOneGridId,
            contentType: "application/json; charset=utf-8",
            dataType: "text",
            success: function (data, textStatus) {
                var d = eval(data);
