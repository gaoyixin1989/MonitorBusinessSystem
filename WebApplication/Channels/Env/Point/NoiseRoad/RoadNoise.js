// Create by 魏林 2013.06.17  "道路交通噪声监测点管理"功能
...
var strTableName = "T_ENV_P_NOISE_ROAD_ITEM";
var strColumnName = "ITEM_ID";
var strWhereColumnName = "POINT_ID";

var strOneGridTitle = "道路交通噪声监测点信息";

var strOneGridAddName = "道路交通噪声监测点信息增加";
var strOneGridUpdateName = "道路交通噪声监测点信息编辑";

var strUrl = "RoadNoise.aspx";
var strOneGridUpdateUrl = "RoadNoiseEdit.aspx";
...
$(document).ready(function () {
    ...
    objOneGrid = $("#oneGrid").ligerGrid({
        ...
        columns: [
                ...
                { display: '道路名称', name: 'ROAD_NAME', minWidth: 180 }
                ],
        toolbar: { items: [
        ...
        '确定', onclick: function (item, dialog) {
            ...
            if (v) {
                ...
                $.ajax({
                    ...
                    data: "{'strTable':'T_ENV_P_NOISE_ROAD','strYear':'" + Year_To + "', 'strMonth':'" + Month_To + "'}",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (data, textStatus) {
                        if (data.d == "1") {
                            ...
                            function (yes) {
                                if (yes == true) {
                                    var rText = $("iframe")[0].contentWindow.DataCopy("type=copyRoadNoise&Year_From=" + Year_From + "&Month_From=" + Month_From + "&Year_To=" + Year_To + "&Month_To=" + Month_To);
                                    var d = eval(rText);
                                    if (d.result) {
                                        dialog.close();
        ...
        if (detailWinSrh) {
            ...
            divmainform.ligerForm({
                ...
                fields: [
                        ...
                        { display: "道路名称", name: "SEA_ROAD", newline: true, type: "text" }
                        ]
            });
    ...
    function search() {
        ...
        var SEA_ROAD = encodeURI($("#SEA_ROAD").val());

        objOneGrid.set('url', "" + strUrl + "?type=getOneGridInfo&srhYear= " + SEA_YEAR + "&srhMonth= " + SEA_MONTH + "&srhName=" + SEA_NAME + "&srhRoadName=" + SEA_ROAD);
...
$(document).ready(function () {
    ...
    function PasteItem() {
        ...
        $.ajax({
            ...
            async: true,
            type: "POST",
            url: "../PointInfoCopy.aspx?type=copyRoadNoiseV&fromID=" + $("#VerticalID").val() + "&toID=" + strOneGridId,
            //data: "{}",
            contentType: "application/json; charset=utf-8",
            dataType: "text",
            success: function (data, textStatus) {
