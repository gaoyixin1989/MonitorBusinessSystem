//created by 刘静楠 2013/6/8 "环境空气监测"
var objOneGrid = null;
var objTwoGrid = null;
...
var strTableName = "T_ENV_P_AIR_ITEM"; //表名:环境空气监测点监测项目表
var strColumnName = "ITEM_ID"; //监测项目ID
var strWhereColumnName = "POINT_ID"; //点位监测ID

var strOneGridTitle = "环境空气监测点信息";
var strOneGridAddName = "环境空气监测点信息增加";
var strOneGridUpdateName = "环境空气监测点信息编辑";

var strUrl = "Environment.aspx";
var strOneGridUpdateUrl = "EnvironmentEditer.aspx";

var intOneGridWidth = 700;
var intOneGridHeight = 400;

var strTwoGridTitle = "环境空气监测点信息项目";
        ...
        '确定', onclick: function (item, dialog) {
            ...
            if (v) {
                ...
                $.ajax({
                    ...
                    data: "{'strTable':'T_ENV_P_AIR','strYear':'" + Year_To + "', 'strMonth':'" + Month_To + "'}",
                    ...
                    success: function (data, textStatus) {
                        if (data.d == "1") {
                            ...
                            function (yes) {
                                if (yes == true) {
                                    var rText = $("iframe")[0].contentWindow.DataCopy("type=copyEnvironment&Year_From=" + Year_From + "&Month_From=" + Month_From + "&Year_To=" + Year_To + "&Month_To=" + Month_To);
                                    var d = eval(rText);
                                    if (d.result) {
                                        dialog.close();
...
$(document).ready(function () {
    objTwoGrid = $("#twoGrid").ligerGrid({
        ...
        heightDiff: -10,
        columns: [
                    { display: '监测项目', name: 'VERTICAL_NAME', align: 'left', width: 135, minWidth: 100, frozen: true, render: function (record) {
    ...
    function PasteItem() {
        ...
        $.ajax({
            ...
            async: true,
            type: "POST",
            url: "../PointInfoCopy.aspx?type=EnvironmentEditer&fromID=" + $("#VerticalID").val() + "&toID=" + strOneGridId,
            contentType: "application/json; charset=utf-8",
            dataType: "text",
            success: function (data, textStatus) {
                var d = eval(data);
