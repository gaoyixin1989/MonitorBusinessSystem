// Create by 魏林 2013.06.14  "生态补偿监测点管理"功能
...
var strTableName = "T_ENV_P_PAYFOR_ITEM";
var strColumnName = "ITEM_ID";
var strWhereColumnName = "POINT_ID";

var strOneGridTitle = "生态补偿监测点信息";

var strOneGridAddName = "生态补偿监测点信息增加";
var strOneGridUpdateName = "生态补偿监测点信息编辑";

var strUrl = "Payfor.aspx";
var strOneGridUpdateUrl = "PayforEdit.aspx";
...
$(document).ready(function () {
    ...
    objOneGrid = $("#oneGrid").ligerGrid({
        ...
        columns: [
                ...
                { display: '断面代码', name: 'POINT_CODE', minWidth: 120 },
                { display: '断面名称', name: 'POINT_NAME', minWidth: 140 },
                { display: '上游断面', name: 'UP_POINT', minWidth: 140 },
                ...
                { display: '控制级别', name: 'CONTRAL_LEVEL', render: function (record) {
                    return getDictName(record.CONTRAL_LEVEL, 'ENV_PAYFOR');
                }
                },
        ...
        '确定', onclick: function (item, dialog) {
            ...
            if (v) {
                ...
                $.ajax({
                    ...
                    data: "{'strTable':'T_ENV_P_PAYFOR','strYear':'" + Year_To + "', 'strMonth':'" + Month_To + "'}",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (data, textStatus) {
                        if (data.d == "1") {
                            $.ligerDialog.confirm("已经存在" + Year_To + "年" + Month_To + "月的点位信息，<br>是否覆盖？<br><br><a style='color:Red'>注意：覆盖之后之前的点位信息将<br>被删除无法恢复</a>",
                            function (yes) {
                                if (yes == true) {
                                    var rText = $("iframe")[0].contentWindow.DataCopy("type=copyPayfor&Year_From=" + Year_From + "&Month_From=" + Month_From + "&Year_To=" + Year_To + "&Month_To=" + Month_To);
                                    var d = eval(rText);
                                    if (d.result) {
                                        dialog.close();
        ...
        if (detailWinSrh) {
            ...
            divmainform.ligerForm({
                ...
                fields: [
                        ...
                        { display: "测点名称", name: "SEA_NAME", newline: true, type: "text" },
                        { display: "控制级别", name: "CONTRAL_LEVEL", newline: true, type: "select", comboboxName: "CONTRAL_LEVEL_BOX", options: { valueFieldID: "CONTRAL_LEVEL_OP", valueField: "DICT_CODE", textField: "DICT_TEXT", url: strOneGridUpdateUrl + "?type=getDict&dictType=ENV_PAYFOR"} }
                        ]
            });
    ...
    function search() {
        ...
        var SEA_LEVEL = encodeURI($("#CONTRAL_LEVEL_OP").val());

        objOneGrid.set('url', "" + strUrl + "?type=getOneGridInfo&srhYear= " + SEA_YEAR + "&srhMonth= " + SEA_MONTH + "&srhName=" + SEA_NAME + "&srhCONTRAL_LEVEL=" + SEA_LEVEL);
    ...
    function clearSearchDialogValue() {
        ...
        $("#SEA_MONTH").val("");
        $("#SEA_NAME").val("");
        $("#CONTRAL_LEVEL_BOX").ligerGetComboBoxManager().setValue("");
    }
...
$(document).ready(function () {
    objTwoGrid = $("#twoGrid").ligerGrid({
        ...
        columns: [
                    ...
                    { display: '考核水质标准值', name: 'STANDARD', width: 120, minWidth: 120, editor: { type: "text"} }
                ],
        toolbar: { items: [
                ...
                { text: '（注：考核标准值可在下表修改）' }
            //{ text: '保存考核标准值', click: ItemSaveSt, icon: 'save'  }
                ]
        },
        onCheckRow: function (checked, rowdata, rowindex) {
            for (var rowid in this.records)
    ...
    function ItemSaveSt() {
        //构建填报数据字符串
        //var gridJSON = objTwoGrid.getData();
        //获取更改过数据的JSON
        var gridUpdateJson = objTwoGrid.getUpdated();
        var data = JSON.stringify(gridUpdateJson);
    ...
    function PasteItem() {
        ...
        $.ajax({
            ...
            async: true,
            type: "POST",
            url: "../PointInfoCopy.aspx?type=copyPayforV&fromID=" + $("#VerticalID").val() + "&toID=" + strOneGridId,
            //data: "{}",
            contentType: "application/json; charset=utf-8",
            dataType: "text",
            success: function (data, textStatus) {
