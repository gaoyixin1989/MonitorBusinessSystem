    ...
    public partial class TransferCustomUI : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (currWorks.Rows.Count == 0)
            {
                ...
                sql = string.Format("SELECT NodeID AS FK_Node,Name AS FK_NodeText, '' as  FK_Emp, '{0}' as FK_EmpText, '" + dt + "' as RDT, 0 as IsPass, Step FROM WF_Node WHERE FK_Flow='{1}' AND NodeID = {2}", WebUser.Name, FK_Flow, FK_Node);
            ...
            //获取流程自定义列表
            sql = string.Format("SELECT wfc.FK_Node,wn.Name AS FK_NodeText,wfc.SubFlowNo, wf.Name AS SubFlowName, wfc.Worker,'' as WorkerText,wfc.Idx + {0} + 1 as Idx FROM WF_TransferCustom wfc INNER JOIN WF_Node wn ON wn.NodeID = wfc.FK_Node LEFT JOIN WF_Flow wf ON wf.No = wfc.SubFlowNo WHERE wfc.WorkID = {1} ORDER BY wfc.Idx ASC", step, WorkID);
            ...
            if (dtTCs.Rows.Count == 0)
            {
                haveNameText = true;
                mustFilter = true;
                sql = string.Format("SELECT wsa.FK_Node,wn.Name AS FK_NodeText,'' as SubFlowNo , '——' as SubFlowName,wsa.FK_Emp AS Worker,EmpName AS WorkerText,wn.Step AS Idx  FROM WF_SelectAccper wsa INNER JOIN WF_Node wn ON wn.NodeID = wsa.FK_Node WHERE wsa.WorkID = {0}", WorkID);
            ...
            if (dtTCs.Rows.Count == 0)
            {
                haveNameText = true;
                mustFilter = true;
                sql = string.Format("SELECT NodeID AS FK_Node,Name AS FK_NodeText, '' as SubFlowNo, '——' as SubFlowName, '' as Worker,'' as WorkerText,Step AS Idx FROM WF_Node WHERE FK_Flow='{0}'", FK_Flow);
                dtTCs = DBAccess.RunSQLReturnTable(sql);
            }
