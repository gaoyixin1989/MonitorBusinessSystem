...
namespace CCFlow.WF.Admin.CCFlowDesigner
{
    ...
    public class FlowDesignerSvr : System.Web.Services.WebService
    {
        ...
        private bool TreeRootCheck()
        {
            ...
            {
                // 流程树根节点校验
                string tmp = "SELECT Name FROM WF_FlowSort where No =0";
                tmp = DBAccess.RunSQLReturnString(tmp);
                if (string.IsNullOrEmpty(tmp))
                {
                    tmp = "INSERT INTO WF_FlowSort(No,Name,ParentNo,TreeNo,idx,IsDir) values('0','流程树','','',0,0)";
                    DBAccess.RunSQLReturnString(tmp);
                }

                // 表单树根节点校验
                tmp = "SELECT Name FROM Sys_FormTree where No =0";
                tmp = DBAccess.RunSQLReturnString(tmp);
                if (string.IsNullOrEmpty(tmp))
                {
                    tmp = "INSERT INTO Sys_FormTree(No,Name,ParentNo,TreeNo,idx,IsDir) values('0','表单树','','',0,0)";
                    DBAccess.RunSQLReturnString(tmp);
                }
