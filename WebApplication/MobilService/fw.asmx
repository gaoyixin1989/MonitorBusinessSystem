<%@ WebService Language="C#" Class="fw" %>
...
public class fw  : System.Web.Services.WebService {
    ...
    public string selLst(string strUserID,string strIfConfirm, string strTaskName, string strSendUser, string strSendTimeBegin, string strSendTimeEnd, int intPageIndex, int intPageSize)
    {
        ...
        DataTable dt = logic.SelectByTableForUserTaskListEx_Mobile(strUserID, "FW",strState, strTaskName, strSendUser, strSendTimeBegin, strSendTimeEnd, intPageIndex, intPageSize);
        int intTotalCount = logic.GetSelectResultCountForUserTaskListEx_Mobile(strUserID, "FW", strState, strTaskName, strSendUser, strSendTimeBegin, strSendTimeEnd);
    ...
    [WebMethod]
    public string getAllFwInfo(string strID, string strWF_INST_ID, string strWF_TASK_ID, string strWF_ID)
    {
        string strReturn = "{";
        strReturn += GetFwInfo(strID, strWF_INST_ID);
        strReturn += ",";
    ...
    private string GetFwInfo(string strID, string strWF_INST_ID)
    {
        ...
        TOaFwInfoVo objFw = new TOaFwInfoLogic().Details(strObjId);
        ...
        else
        {
            ...
            strMobileFileName = "FW" + strObjId + strDatetimeStr + TOaAttVoTemp.ATTACH_TYPE;
            try
            {
        ...
        string strReturn = "";
        strReturn += "'发文标题':'" + objFw.FW_TITLE + "',";
        strReturn += "'主办单位':'" + objFw.ZB_DEPT + "',";
        strReturn += "'主送单位':'" + objFw.ZS_DEPT + "',";
        strReturn += "'抄报单位':'" + objFw.CB_DEPT + "',";
        strReturn += "'抄送单位':'" + objFw.CS_DEPT + "',";
        strReturn += "'拟稿人':'" + new TSysUserLogic().Details(objFw.DRAFT_ID).REAL_NAME + "',";
        strReturn += "'主题词':'" + objFw.SUBJECT_WORD + "',";
        strReturn += "'密级':'" + new TSysDictLogic().Details(new TSysDictVo() { DICT_TYPE = "FW_MJ", DICT_CODE = objFw.MJ }).DICT_TEXT + "',";
    ...
    public string getAppInfo(string strID, string strWF_INST_ID)
    {
        ...
        string strReturn = "[";
        strReturn += "{";
        strReturn += "'step':'办公室审核',";
        strReturn += "'appInfo':'" + objFw.APP_INFO + "',";
        strReturn += "'UserName':'" + new TSysUserLogic().Details(objFw.APP_ID).REAL_NAME + "',";
        strReturn += "'appTime':'" + objFw.APP_DATE + "'";
        strReturn += "},";
        strReturn += "{";
        strReturn += "'step':'分管领导审核',";
        strReturn += "'appInfo':'" + objFw.CTS_INFO + "',";
        strReturn += "'UserName':'" + new TSysUserLogic().Details(objFw.CTS_ID).REAL_NAME + "',";
        strReturn += "'appTime':'" + objFw.CTS_DATE + "'";
        strReturn += "},";
        strReturn += "{";
        strReturn += "'step':'主要领导签发',";
        strReturn += "'appInfo':'" + objFw.ISSUE_INFO + "',";
        strReturn += "'UserName':'" + new TSysUserLogic().Details(objFw.ISSUE_ID).REAL_NAME + "',";
        strReturn += "'appTime':'" + objFw.ISSUE_DATE + "'";
        strReturn += "}";
        strReturn += "]";
    ...
    public string sendTask(string strID, string strWF_INST_ID, string strWF_ID, string strWF_TASK_ID, string strUserID, string strAppInfo,  string strSendUserId)
    {
        ...
        //------------ 发文信息 start
        bool isSuccess = saveFwWhenSendOrBack(strObjId, strIsSendOrBack, strAppInfo, strUserID, ref strSERVICE_ROW_SIGN);
        //------------ 发文信息 end
    ...
    private bool saveFwWhenSendOrBack(string strObjId, string strIsSendOrBack, string strAppInfo, string strUserID, ref string strSERVICE_ROW_SIGN)
    {
        TOaFwInfoVo objFW = new TOaFwInfoLogic().Details(strObjId);
        string strFwStatus = objFW.FW_STATUS;

        objFW = new TOaFwInfoVo();
        objFW.ID = strObjId;
        if (strFwStatus == "1")//办公室审核
        {
            objFW.APP_INFO = strAppInfo;
            objFW.APP_ID = strUserID;
            objFW.APP_DATE = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            strSERVICE_ROW_SIGN = strIsSendOrBack == "1" ? "2" : "0";
            objFW.FW_STATUS = strSERVICE_ROW_SIGN;
        }

        if (strFwStatus == "2")//分管领导审核
        {
            objFW.CTS_INFO = strAppInfo;
            objFW.CTS_ID = strUserID;
            objFW.CTS_DATE = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            strSERVICE_ROW_SIGN = strIsSendOrBack == "1" ? "3" : "1";
            objFW.FW_STATUS = strSERVICE_ROW_SIGN;
        }
        if (strFwStatus == "3")//主要领导签发
        {
            objFW.ISSUE_INFO = strAppInfo;
            objFW.ISSUE_ID = strUserID;
            objFW.ISSUE_DATE = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            strSERVICE_ROW_SIGN = strIsSendOrBack == "1" ? "4" : "2";
            objFW.FW_STATUS = strSERVICE_ROW_SIGN;
        }
        return new TOaFwInfoLogic().Edit(objFW);
    }
