<%@ WebService Language="C#" Class="WfTechApp" %>
...
public class WfTechApp  : System.Web.Services.WebService {
    ...
    public string SendTask(string strID, string strUserId)
    {
        ...
        //记录当前分析环节的发送人
        DataTable objTable = new TMisMonitorResultLogic().geResultChecktItemTypeInfo_QHD(strUserId, "qc_manager_audit", strTaskId, "03", "0", "60");
        ...
        if (IsSuccess == true)//郑州增加技术负责人审核，质量负责人审核不再是最后一环节
        {
            //如果是环境质量将自动对数据进行填报
            foreach (DataRow row in objTable.Rows)
            {
    ...
    {
        bool IsSuccess = new TMisMonitorResultLogic().TaskGoBackInQcCheck_QHD(strID, strUserId, "qc_manager_audit", "0", "60", "50");
        return IsSuccess == true ? "1" : "0";
    }
