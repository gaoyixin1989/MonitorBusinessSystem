...
public partial class Sys_WF_WFShowStepImg : PageBase
{
    ...
    private Table GetResultRow(Table table, string strYesDivStyle, string strWaitDivStyle, string strStepName, DataTable dtStepInfo)
    {
        ...
        foreach (DataRow dr in dtStepInfo.Rows)
        {
            ...
            {
                strNoSumit += GetSampleManager(dr["HEAD_USERID"].ToString()) + "、";
            }
            else//结果已提交
            {
                strYesSumit += GetSampleManager(dr["HEAD_USERID"].ToString()) + "、";
            }
        }
