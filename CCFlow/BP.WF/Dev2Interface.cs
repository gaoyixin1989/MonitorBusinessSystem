...
namespace BP.WF
{
    ...
    public class Dev2Interface
    {
        ...
        public static DataTable DB_GenerDraftDataTable()
        {
            ...
            ps.SQL = "SELECT WorkID,Title,FK_Flow,FlowName,RDT FROM WF_GenerWorkFlow A WHERE WFState=1 AND Starter=" + dbStr + "Starter ORDER BY RDT";
        ...
        {
            string dbStr = BP.Sys.SystemConfig.AppCenterDBVarStr;
            WorkNode wn = new WorkNode(workid, nodeFrom);
            //待办抢办模式，一个节点只能有一条记录
            Paras ps = new Paras();
        ...
        public static void WriteTrackWeekLog(string flowNo, int nodeFrom, Int64 workid, Int64 fid, string msg, string optionName)
        {
            ...
            {
                Paras ps = new Paras();
                ps.SQL = "UPDATE  ND" + int.Parse(flowNo) + "Track SET Msg=" + dbStr + "Msg WHERE  RDT LIKE '" + insertDate + "%' AND WorkID=" + dbStr + "WorkID  AND NDFrom=" + dbStr + "NDFrom AND EmpFrom=" + dbStr + "EmpFrom AND ActionType=" + (int)ActionType.WorkCheck;
                ps.Add(TrackAttr.Msg, msg);
                ps.Add(TrackAttr.WorkID, workid);
                ps.Add(TrackAttr.NDFrom, nodeFrom);
                ps.Add(TrackAttr.EmpFrom, WebUser.No);
        ...
        public static bool Flow_IsCanStartThisFlow(string flowNo, string userNo)
        {
            ...
            switch (nd.HisDeliveryWay)
            {
                ...
                case DeliveryWay.ByDept:
                    ps.SQL = "SELECT COUNT(A.FK_Node) as Num FROM WF_NodeDept A, Port_EmpDept B WHERE A.FK_Dept= B.FK_Dept AND  A.FK_Node=" + dbstr + "FK_Node AND B.FK_Emp=" + dbstr + "FK_Emp";
                    ps.Add("FK_Node", nd.NodeID);
                    ps.Add("FK_Emp", userNo);
                    num = DBAccess.RunSQLReturnValInt(ps);
                    break;
