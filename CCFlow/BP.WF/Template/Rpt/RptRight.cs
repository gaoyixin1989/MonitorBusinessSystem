...
namespace BP.WF.Rpt
{
    ...
    /// 报表设计
    /// </summary>
    public class RptRightAttr : EntityMyPKAttr
    {
    ...
    /// 报表设计
    /// </summary>
    public class RptRight : EntityNoName
    {
        #region 报表权限控制方式
        /// <summary>
        /// 报表查看权限控制.
        ...
        public RightViewWay RightViewWay
        {
            get
            {
                return (RightViewWay)this.GetValIntByKey(RptRightAttr.RightViewWay);
            }
            set
            {
                this.SetValByKey(RptRightAttr.RightViewWay, (int)value);
            }
        }
        /// <summary>
        /// 报表查看权限控制-数据
        ...
        public string RightDeptTag
        {
            get
            {
                return this.GetValStringByKey(RptRightAttr.RightDeptTag);
            }
            set
            {
                this.SetValByKey(RptRightAttr.RightDeptTag, value);
            }
        }
        /// <summary>
        /// 报表部门权限控制.
        ...
        public RightDeptWay RightDeptWay
        {
            get
            {
                return (RightDeptWay)this.GetValIntByKey(RptRightAttr.RightDeptWay);
            }
            set
            {
                this.SetValByKey(RptRightAttr.RightDeptWay, (int)value);
            }
        }
        /// <summary>
        /// 报表部门权限控制-数据
        ...
        public string FK_Flow
        {
            get
            {
                string s = this.GetValStrByKey(RptRightAttr.FK_Flow);
                if (s == "" || s == null)
                {
            ...
            set
            {
                this.SetValByKey(RptRightAttr.FK_Flow, value);
            }
        }
        public string PTable
        {
            get
            {
                string s = this.GetValStrByKey(RptRightAttr.PTable);
                if (s == "" || s == null)
                    return this.No;
                return s;
            }
            set
            {
                this.SetValByKey(RptRightAttr.PTable, value);
            }
        }
        /// <summary>
        /// 备注
        ...
        public string Note
        {
            get
            {
                return this.GetValStrByKey(RptRightAttr.Note);
            }
            set
            {
                this.SetValByKey(RptRightAttr.Note, value);
            }
        }
        public string ParentMapData
        {
            get
            {
                return this.GetValStrByKey(RptRightAttr.ParentMapData);
            }
            set
            {
                this.SetValByKey(RptRightAttr.ParentMapData, value);
            }
        }
        ...
        public RptRight(string no, string flowNo)
        {
            this.No = no;
            this.Retrieve();
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBStringPK(RptRightAttr.No, null, "编号", true, false, 1, 20, 20);
                map.AddTBString(RptRightAttr.Name, null, "描述", true, false, 0, 500, 20);
                //     map.AddTBString(RptRightAttr.SearchKeys, null, "查询键", true, false, 0, 500, 20);
                map.AddTBString(RptRightAttr.PTable, null, "物理表", true, false, 0, 500, 20);
                map.AddTBString(RptRightAttr.FK_Flow, null, "流程编号", true, false, 0, 3, 3);

                //Tag
                //   map.AddTBString(RptRightAttr.Tag, null, "Tag", true, false, 0, 500, 20);

                //时间查询:用于报表查询.
                //  map.AddTBInt(RptRightAttr.IsSearchKey, 0, "是否需要关键字查询", true, false);
                //   map.AddTBInt(RptRightAttr.DTSearchWay, 0, "时间查询方式", true, false);
                //   map.AddTBString(RptRightAttr.DTSearchKey, null, "时间查询字段", true, false, 0, 200, 20);
                map.AddTBString(RptRightAttr.Note, null, "备注", true, false, 0, 500, 20);

                map.AddTBString(RptRightAttr.ParentMapData, null, "ParentMapData", true, false, 0, 128, 20);


                #region 权限控制. 2014-12-18
                map.AddTBInt(RptRightAttr.RightViewWay, 0, "报表查看权限控制方式", true, false);
                map.AddTBString(RptRightAttr.RightViewTag, null, "报表查看权限控制Tag", true, false, 0, 4000, 20);

                map.AddTBInt(RptRightAttr.RightDeptWay, 0, "部门数据查看控制方式", true, false);
                map.AddTBString(RptRightAttr.RightDeptTag, null, "部门数据查看控制Tag", true, false, 0, 4000, 20);
    ...
    /// 报表设计s
    /// </summary>
    public class RptRights : EntitiesMyPK
    {
        #region 构造
        /// <summary>
        /// 报表设计s
        ...
        public RptRights(string fk_flow)
        {
            string fk_Mapdata = "ND" + int.Parse(fk_flow) + "Rpt";
            int i = this.Retrieve(RptRightAttr.ParentMapData, fk_Mapdata);
            if (i == 0)
            {
                MapData mapData = new MapData(fk_Mapdata);
                ...
                RptRight rpt = new RptRight(mapData.No, fk_flow);
